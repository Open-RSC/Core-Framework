...
public class CacheUpdater extends Activity {
	...
	public static final String CACHE_URL = "http://www.openrsc.com/client/";
		...
		boolean verifyFile(String filename, String checksum) {
			try {
				...
				if (downloadedChecksum.equalsIgnoreCase(checksum)) {
					publishProgress("Verified: " + (filename + " hash" + downloadedChecksum
							+ " = " + checksum + "-> OK \n"));

					Log.d("Updater", (filename + " hash" + downloadedChecksum + " = "
							+ checksum + "-> OK"));
					return true;
				}
				Log.d("Updater", filename + " hash:" + downloadedChecksum
						+ " doesn't match MD5: " + checksum + " re-downloading");
				publishProgress("Verification failed, re-downloading " + filename + "...");
				publishProgress(
						"Downloaded file: " + filename + " hash:" + downloadedChecksum
