...
public class ApplicationUpdater extends Activity {
				...
				.setCancelable(false).setPositiveButton("Install", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						try {
							...
							File downloadedFile = new File(getFilesDir() + File.separator + "droid.apk");
	...
	class DownloadApplication extends AsyncTask<String, String, String> {
		...
		protected String doInBackground(String... aurl) {
			...
			try {
				connection = (HttpURLConnection) new URL(ANDROID_CACHE_URL + "droid.apk").openConnection();
				connection.connect();

				int fileLength = connection.getContentLength();
				FileOutputStream fos = openFileOutput("droid.apk", Context.MODE_PRIVATE);
				try {
