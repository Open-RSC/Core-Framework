...
import rsc.graphics.gui.MenuItem;
...
public final class ArrayUtil {
	...
	public static final void alignedCopy(byte[] src, int sI, byte[] dest, int dI, int count) {
		if (src == dest) {
			if (sI == dI) {
				return;
			}
	...
	private static final void quickSort(MenuItem[] items, int[] priority, int left, int right) {
		try {
			if (right > left) {
	...
	public static final void quickSort(MenuItem[] items, int[] priority) {
		try {
