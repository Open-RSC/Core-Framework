...
public class PacketHandler {
    ...
    public final void handlePacket2(int opcode, int length) {
        try {
            ...
            StringBuilder var5 = new StringBuilder("T2 - " + opcode + " - " + length + " rx:" + mc.getLocalPlayerX() + " ry:" + mc.getLocalPlayerZ()
                    + " num3l:" + mc.getGameObjectInstanceCount() + " - ");

            for (int var6 = 0; length > var6 && var6 < 50; ++var6) {
                var5.append(packetsIncoming.getByte()).append(",");
            }
                ...
                if (SocialLists.friendList[i].equals(currentName)) {
                    if (SocialLists.friendListArgS[i] == null && online) {
                        mc.showMessage(false, null, currentName + " has logged in",
                                MessageType.FRIEND_STATUS, 0, null, null);
                    }
    ...
    private void sendPrivateMessage() {
        ...
        mc.showMessage(false, var13, var14, MessageType.PRIVATE_SEND, 0, var13, null);
    }
        ...
        while (length > packetsIncoming.packetEnd) {
            if (packetsIncoming.getUnsignedByte() != 255) {
                ...
                if (id != 60000) {
                    int xSize, zSize;
                    if (dir == 0 || dir == 4) {
                        zSize = Objects.requireNonNull(EntityHandler.getObjectDef(id)).getHeight();
                        xSize = Objects.requireNonNull(EntityHandler.getObjectDef(id)).getWidth();
                    } else {
                        xSize = Objects.requireNonNull(EntityHandler.getObjectDef(id)).getHeight();
                        zSize = Objects.requireNonNull(EntityHandler.getObjectDef(id)).getWidth();
                    }

                    int tileSize = mc.getTileSize();
                    int xWorld = (xTile * 2 + xSize) * tileSize / 2;
                    int zWorld = (zTile * 2 + zSize) * tileSize / 2;
                    int modelIndex = Objects.requireNonNull(EntityHandler.getObjectDef(id)).modelID;// CacheValues.gameObjectModelIndex[id];
    ...
    private void updateInventoryItems() {
        ...
        for (int i = 0; i < mc.getInventoryItemCount(); ++i) {
            ...
            if (Objects.requireNonNull(EntityHandler.getItemDef(itemID)).isStackable()) {
                ...
                if (npc != null) {
                    ...
                    if (mc.getLocalPlayer().serverIndex == chatRecipient) {
                        mc.showMessage(false, null,
                                Objects.requireNonNull(EntityHandler.getNpcDef(npc.npcId)).getName() + ": "
                                        + npc.message,
    ...
    private void updateInventory() {
        ...
        int itemID = packetsIncoming.getShort();
        int stackSize = 1;
        if (Objects.requireNonNull(EntityHandler.getItemDef(itemID & 32767)).isStackable()) {
    ...
    private void drawNearbyPlayers() {
        ...
        for (int pp = 0; playerCount > pp; ++pp) {
            ...
            if (updateType == 0) {
                ...
                if (null != player) {
                    ...
                    if (!var29) {
                        ...
                        mc.showMessage(crownID == 2, (player.clanTag != null ? "@whi@[@cla@" + player.clanTag + "@whi@]@yel@ " + player.displayName : player.displayName), player.message,
                                MessageType.CHAT, crownID, player.accountName, null);
                    }
                }
                ...
                player.message = packetsIncoming.readString();
                player.messageTimeout = 150;
                if (mc.getLocalPlayer() == player) {
                    ...
                    for (int var7 = 0; mc.getGameObjectInstanceCount() > var7; ++var7) {
                        if (mc.getGameObjectInstanceX(var7) == var19
                            ...
                            mc.setGroundItemHeight(mc.getGroundItemCount(),
                                    Objects.requireNonNull(EntityHandler.getObjectDef(
