...
public class Model
{
	...
	public Model( byte abyte0[], int i, boolean flag )
	{
		...
		for( int l2 = 0; l2 < k; l2++ )
		{
			...
			for( int i3 = 0; i3 < anIntArray235[l2]; i3++ )
				...
				{
					anIntArrayArray236[l2][i3] = DataOperations
							.getUnsigned2Bytes( abyte0, i );
					i += 2;
				}
		...
		for( int i1 = 0; i1 < i; i1++ )
		{
			...
			for( int j1 = 0; j1 < model.anInt234; j1++ )
			{
				...
				for( int k1 = 0; k1 < model.anIntArray235[j1]; k1++ )
					ai[k1] = method179( model.anIntArray272[ai1[k1]],
							model.anIntArray273[ai1[k1]],
							model.anIntArray274[ai1[k1]] );

				int l1 = method181( model.anIntArray235[j1], ai,
						model.anIntArray237[j1], model.anIntArray238[j1] );
	...
	public int method179( int i, int j, int k )
	{
		for( int l = 0; l < anInt226; l++ )
			if( anIntArray272[l] == i && anIntArray273[l] == j
					&& anIntArray274[l] == k )
				return l;
	...
	public Model[] method182( int i, int j, int k, int l, int i1, int j1,
			int k1, boolean flag )
	{
		method202();
		int ai[] = new int[j1];
		...
		for( int l2 = 0; l2 < j1; l2++ )
		{
			if( ai[l2] > k1 )
				ai[l2] = k1;
			models[l2] = new Model( ai[l2], ai1[l2], true, true, true, flag,
					true );
	...
	public void method183( Model model, int ai[], int i, int j )
	{
		...
		for( int k = 0; k < i; k++ )
		{
			int l = ai1[k] = model.method179( anIntArray272[ai[k]],
					anIntArray273[ai[k]], anIntArray274[ai[k]] );
	...
	{
		if( anInt295 != 256 || anInt296 != 256 || anInt297 != 256
				|| anInt298 != 256 || anInt299 != 256 || anInt300 != 256 )
		{
			anInt301 = 4;
			return;
		}
	...
	private void method194( int i, int j, int k )
	{
		for( int i3 = 0; i3 < anInt226; i3++ )
		{
			if( k != 0 )
			{
				...
				anIntArray276[i3] = anIntArray276[i3] * k1 - anIntArray275[i3]
						* l >> 15;
				anIntArray275[i3] = j2;
			}
			if( i != 0 )
			{
				...
				anIntArray277[i3] = anIntArray276[i3] * i1 + anIntArray277[i3]
						* l1 >> 15;
				anIntArray276[i3] = k2;
			}
			if( j != 0 )
			{
				...
				anIntArray277[i3] = anIntArray277[i3] * i2 - anIntArray275[i3]
						* j1 >> 15;
				anIntArray275[i3] = l2;
			}
		}
	...
	public void method198()
	{
		...
		for( int j = 0; j < anInt234; j++ )
			if( anIntArray241[j] != anInt270 )
				anIntArray241[j] = (anIntArray242[j] * anInt303
						+ anIntArray243[j] * anInt304 + anIntArray244[j]
						* anInt305)
						/ i;
		...
		for( int j1 = 0; j1 < anInt226; j1++ )
			if( ai3[j1] > 0 )
				anIntArray232[j1] = (ai[j1] * anInt303 + ai1[j1] * anInt304 + ai2[j1]
						* anInt305)
						/ (i * ai3[j1]);
	...
	public void method199()
	{
		...
		for( int i = 0; i < anInt234; i++ )
		{
			...
			int l2 = k1 * l1 - j2 * i1;
			int i3;
			for( i3 = i1 * i2 - l1 * j1; k2 > 8192 || l2 > 8192 || i3 > 8192
					|| k2 < -8192 || l2 < -8192 || i3 < -8192; i3 >>= 1 )
			{
				k2 >>= 1;
				l2 >>= 1;
			}
	...
	public void method200()
	{
		...
		if( anInt246 == 1 )
		{
			...
			if( anInt301 >= 4 )
				method195( anInt295, anInt296, anInt297, anInt298, anInt299,
						anInt300 );
			if( anInt301 >= 1 )
	...
	public Model method204( boolean flag, boolean flag1, boolean flag2,
			boolean flag3 )
	{
		Model models[] = new Model[1];
		models[0] = this;
	...
	static
	{
		...
		for( int j = 0; j < 1024; j++ )
		{
			...
			anIntArray266[j + 1024] = (int) (Math
					.cos( (double) j * 0.00613592315D ) * 32768D);
		}
