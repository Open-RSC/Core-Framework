 ...
 */
class SpriteDrawer extends SpriteHandler {
	/**
	 ...
	 */
	SpriteDrawer(Component component, int width, int height) {
	 ...
	 */
	void drawSprite(int x, int y, Sprite sprite, int overlay) {
		...
		mergePixels(imagePixelArray, sprite.getPixels(), i1, l, spriteWidth, spriteHeight, l1, i2, overlay);
	}

	/**
	 * Merges the given pixels
	 */
	private void mergePixels(int[] existingPixels, int[] newPixels, int j, int k, int spriteWidth, int spriteHeight, int j1, int k1, int overlay) {
		int[] overData = overlay != 0 ? new int[]{overlay & 0xff, overlay >> 8 & 0xff, overlay >> 16 & 0xff} : null;

		for (int y = -spriteHeight; y < 0; y += 1) {
