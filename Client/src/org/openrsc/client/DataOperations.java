...
public class DataOperations {
   ...
   public static Image getImage(byte data[], int offset, int length)
    {
    ...
    public static int getOffset(String fileName, byte dataStream[]) {
        int entryCount = getUnsigned2Bytes(dataStream, 0);
        int hash = 0;
        fileName = fileName.toUpperCase();
        for (int ptr = 0; ptr < fileName.length(); ptr++)
            hash = (hash * 61 + fileName.charAt(ptr)) - 32;

        int offset = 2 + entryCount * 10;
        for (int entryPointer = 0; entryPointer < entryCount; entryPointer++)
        {
            int entryHash = (dataStream[entryPointer * 10 + 2] & 0xff) * 0x1000000 + (dataStream[entryPointer * 10 + 3] & 0xff) * 0x10000 + (dataStream[entryPointer * 10 + 4] & 0xff) * 256 + (dataStream[entryPointer * 10 + 5] & 0xff);
            int entryLength = (dataStream[entryPointer * 10 + 9] & 0xff) * 0x10000 + (dataStream[entryPointer * 10 + 10] & 0xff) * 256 + (dataStream[entryPointer * 10 + 11] & 0xff);
            if (entryHash == hash)
                return offset;
            offset += entryLength;
        }
	...
	public static byte[] load(java.io.InputStream inputstream) {
		int j = 0;
		int k = 0;
		byte abyte0[] = null;
		try {
	...
	public static int getLength(String s, byte abyte0[]) {
        int entryCount = getUnsigned2Bytes(abyte0, 0);
        int hash = 0;
        s = s.toUpperCase();
        for (int k = 0; k < s.length(); k++)
            hash = (hash * 61 + s.charAt(k)) - 32;

        for (int i1 = 0; i1 < entryCount; i1++) {
            int entryHash = (abyte0[i1 * 10 + 2] & 0xff) * 0x1000000 + (abyte0[i1 * 10 + 3] & 0xff) * 0x10000 + (abyte0[i1 * 10 + 4] & 0xff) * 256 + (abyte0[i1 * 10 + 5] & 0xff);
            int entryLength = (abyte0[i1 * 10 + 6] & 0xff) * 0x10000 + (abyte0[i1 * 10 + 7] & 0xff) * 256 + (abyte0[i1 * 10 + 8] & 0xff);
            if (entryHash == hash)
                return entryLength;
        }
