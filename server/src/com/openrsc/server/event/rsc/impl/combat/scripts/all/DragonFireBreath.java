	...
	public void executeScript(Mob attacker, Mob victim) {
		...
		if (attacker.isNpc() || victim.getID() == NpcId.DRAGON.id() || victim.getID() == NpcId.KING_BLACK_DRAGON.id()) {
			...
			if (player.isRanging() && (dragon.getID() == NpcId.DRAGON.id() || dragon.getID() == NpcId.KING_BLACK_DRAGON.id())) {
				...
				fireDamage = (int) Math.floor(getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) * percentage / 100.0);
			} else {
				fireDamage = Math.min(getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)), DataConversions.random(0, maxHit));
			}
			if (fireDamage >= 25 || (fireDamage >= 20 && getMaxLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) * 2/5 < 25)) {
			...
			if (dragon.getID() == NpcId.KING_BLACK_DRAGON.id() && (player.isRanging() || attacker.isPlayer())) {
				int newLevel = getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.RANGED)) - Formulae.getLevelsToReduceAttackKBD(player);
				player.getSkills().setLevel(getSkillId(player.getWorld(), SkillsEnum.RANGED), newLevel);
			}
		}
	}
