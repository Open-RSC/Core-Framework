...
public class PkBotBehavior extends NpcBehavior {
	...
	private synchronized void attemptTackle(Npc n, Player player) {
	...
	private boolean canAggro(Mob player) {
		...
		boolean shouldAttack = (npc.getDef().isAggressive() && (player.getCombatLevel() < ((npc.getNPCCombatLevel() * 2) + 1)
			|| npc.getLocation().inWilderness())) || (npc.getLastOpponent() == player && !shouldRetreat(npc));
	...
	private boolean canAggro28(final Mob player) {
		boolean outOfBounds = !player.getLocation().inBounds(npc.getLoc().minX - 4, npc.getLoc().minY - 4,
			npc.getLoc().maxX + 4, npc.getLoc().maxY + 4);
		...
		return closeEnough && shouldAttack
			&& (player instanceof Npc) && !outOfBounds && !playerOccupied && !playerCombatTimeout;
	}
