...
/**
 * A type of {@link EventListener} that is invoked when a {@link Player} uses 
 * an {@link InvItem} on a {@link Item}
 * 
 * @author Zilent
 * 
 * @version 1.0
 ...
 *
 */
public interface UseItemOnGroundItemListener
	extends
		EventListener
{
	/**
	 * Invoked when the provided {@link Player} attempts to use the 
	 * {@link InvItem} at the provided index on the provided {@link Item}
	 * 
	 * @param player the {@link Player} that is using the item
	 * 
	 * @param index the index of the item that is being used
	 * 
	 * @param item the {@link Item} that the item is being used on
	 * 
	 * @return true if this listener has successfully handled the event, 
	 * otherwise false
	 * 
	 */
	boolean onItemUsedOnGroundItem(Player player, int index, Item item);
}
