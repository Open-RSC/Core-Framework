...
public class InvUseOnObject implements PacketHandler {
		...
		if (player != null) {
			...
			switch (pID) {
				...
				case 58: // Use Item on GameObject
					...
					handleObject(player, x, y, object, item);
					break;
			}
		}
	...
	private void handleObject(final Player player, int x, int y, final GameObject object, final InvItem item) {
  				...
  				World.getDelayedEventHandler().add(new ShortEvent(owner) {
					...
					if (owner.getInventory().remove(item) > -1) {
						if (!Formulae.burnFood(item.getID(), owner.getCurStat(7))) {
							...
							owner.increaseXP(Skills.COOKING, cookingDef.getExp());
							owner.sendStat(7);
						} else {
