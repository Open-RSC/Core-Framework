...
// This class is clean
public class Wizard implements NpcHandler {
	...
	private void whoAreYou(final Npc npc, final Player owner) {
		World.getDelayedEventHandler().add(new DelayedQuestChat(npc, owner, new String[] {"I am a wizard, a master of magic", "Why are you here adventurer?"}) {
			public void finished() {
				World.getDelayedEventHandler().add(new SingleEvent(owner, 1500) {
					public void action() {
						final String[] options110 = {"I'm in search of a quest", "What are you doing here?", "No reason, I'll be on my way now"};
						owner.setBusy(false);
						owner.sendMenu(options110);
						owner.setMenuHandler(new MenuHandler(options110) {
							public void handleReply(final int option, final String reply) {
								owner.setBusy(true);
								for(Player informee : owner.getViewArea().getPlayersInView()) {
									informee.informOfChatMessage(new ChatMessage(owner, reply, npc));
		...
		World.getDelayedEventHandler().add(new DelayedQuestChat(npc, owner, new String[] {"Look pal, theres no way I'm giving you any information"}) {
			public void finished() {
				World.getDelayedEventHandler().add(new DelayedQuestChat(owner, npc, new String[] {"Please?"}) {
					public void finished() {
						World.getDelayedEventHandler().add(new DelayedQuestChat(npc, owner, new String[] {"Get lost before I turn you to ash.."}) {
							public void finished() {
								owner.setBusy(false);
								npc.unblock();
