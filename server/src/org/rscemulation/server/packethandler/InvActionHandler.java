						...
						switch (option) {
							case 0: // Yes, subscribe.
								if (player.getInventory().remove(1304, 1) != -1) {
									...
									player.sendAlert("Thank you for subscribing to RSCUnity!  Without subscribers we simply wouldn't be able to keep RSCUnity up and running.  You have " + player.getDaysSubscriptionLeft() + " days remaining.");
	...
	private void bury(Player player, final InvItem item) {
		...
		World.getDelayedEventHandler().add(new MiniEvent(player) {
			public void action() {
				...
				switch(item.getID()) {
					case 20: // Bones
						owner.increaseXP(5, 3.5, 1);
						break;						
					case 604: // Bat Bones
						owner.increaseXP(5, 4, 1);
						break;
					case 413: // Big Bones
						owner.increaseXP(5, 12, 1);
						break;
					case 814: // Dragon Bones
						owner.increaseXP(5, 60, 1);
						break;
				}
				owner.sendStat(5);
				owner.sendInventory();
