	...
	public void handlePacket(Packet p, IoSession session) throws Exception {
		...
		if (player != null) {
			if (!player.isBusy()) {
				...
				// but in RSCE's case...we can add in a little clean-up when the item disappears.
							...
							if (!(owner.isRemoved() || owner.isBusy() || owner.isRanging() || !owner.nextTo(item) || owner.getStatus() != Action.TAKING_GITEM)) {
								...
								if (owner.getInventory().canHold(invItem)) {
									...
									case 412: // Skull [QUEST]
										if (owner.getQuestCompletionStage(5) == 2 || owner.getQuestCompletionStage(5) == 3 && !owner.getInventory().contains(412)) {
