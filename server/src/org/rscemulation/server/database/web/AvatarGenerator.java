...
package org.rscemulation.server.database.web;
...
import org.rscemulation.server.database.Transaction;
import org.rscemulation.server.model.PlayerAppearance;
import org.rscemulation.server.util.DataConversions;

/**
 * A simple avatar generation utility.  This tool takes a player's
 * appearance and creates *.png image from it, then stores that image 
 * to the database.  All internal operations are executed asynchronously, 
 * meaning that the expensive image processing should have a migitated 
 * effect on the game server.
 * 
 * @author Zilent
 * 
...
public final class AvatarGenerator
{
	...
	private final static int AVATAR_HEIGHT = 128;
			...
			Transaction
	{
	    ...
	    private final static AnimationDef[] animations = 
    	{
    			...
    			new AnimationDef("head3", 1, 5, true, false, 0), 
    			...
    			new AnimationDef("partyhat", 65535, 0, true, false, 0), 
    			new AnimationDef("dragon", 16711680, 0, true, false, 0), 
    			new AnimationDef("hatchet", 16737817, 0, true, false, 0), 
		...
		static
		{
			try
			{
				ZipFile spritesArchive = new ZipFile("config/sprites");
				int animationNumber = 0;
				label0: for (int animationIndex = 0; animationIndex < animations.length; animationIndex++)
				{
	    ...
	    private final void drawPlayer(int x, int y, int scaleX, int scaleY, int unknown)
	    {
	        for (int k2 = 0; k2 < 12; k2++)
	        {
	            ...
	            if (animationIndex >= 0)
	            {
	                ...
	                int i5 = 0;
	                int j5 = 0;
                    int k5 = j5 + animations[animationIndex].getNumber();
	    ...
	    /// The user of the player to generate an avatar of
		...
		/// Sole Constructor
		public AvatarTransaction(long user, PlayerAppearance appearance, int[] wornItems)
		{
			this.user = user;
		...
		public Integer call()
			throws
				Exception
		{
			drawPlayer(0, 0, AVATAR_WIDTH, AVATAR_HEIGHT, 0);
		...
		{
			return "\"Avatar Update\" {user=" + DataConversions.hashToUsername(user) + "}";
		}
	...
	public void generateAvatar(long user, PlayerAppearance appearance, int[] wornItems)
	{
		if(appearance == null)
		{
		...
		databaseService.submit(new AvatarTransaction(user, appearance, wornItems));
	}
}
