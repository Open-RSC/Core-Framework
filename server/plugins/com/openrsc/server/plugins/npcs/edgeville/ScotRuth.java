...
public class ScotRuth implements
	...
	public void onTalkNpc(Player p, Npc n) {
		if (p.getCache().hasKey("scotruth_to_chaos_altar")) {
			...
			int choice = multi(p, n, "Yes", "No, 200,000gp is a rip-off");
			if (choice == 0) {
				if (ifheld(p, ItemId.COINS.id(), 200000)) {
					npcsay(p, n, "Excellent. Keep in mind ye'll appear in the deep wilderness",
					...
					npcsay(p, n, "Come back with the money and you've a deal, lad");
				}
			} else if (choice == 1) {
				npcsay(p, n, "Suit yerself.");
			}
		}
	}
