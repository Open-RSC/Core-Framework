		...
		if (n.getID() == IRON_MAN || n.getID() == ULTIMATE_IRON_MAN || n.getID() == HARDCORE_IRON_MAN) {
			if (player.getAttribute("ironman_delete", false)) {
				if (player.getCache().hasKey("bank_pin")) {
					mes(player, n, config().GAME_TICK * 2, "Enter your Bank PIN to downgrade your Iron Man status.");
				...
				mes(player, n, config().GAME_TICK * 2, "You'll need to set a Bank PIN for that.");
