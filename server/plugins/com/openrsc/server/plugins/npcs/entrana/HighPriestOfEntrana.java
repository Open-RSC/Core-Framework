...
public class HighPriestOfEntrana implements TalkNpcTrigger {
	...
	private static void entranaPriestDialogue(Player p, Npc n, int cID) {
		if (cID == -1) {
			npcsay(p, n, "Many greetings welcome to our fair island");
			if (p.getQuestStage(Quests.THE_HOLY_GRAIL) >= 2) {
				say(p, n, "Hello, I am in search of the holy grail");
				npcsay(p, n, "The object of which you speak did once pass through holy entrana",
					"I know not where it is now",
					"Nor do I really care");
				n = ifnearvisnpc(p, NpcId.CRONE.id(), 20);
				npcsay(p, n, "Wait!",
					"Did you say the grail?",
					"You are a grail knight yes?",
					"Well you'd better hurry, a fisher king is in pain");
				say(p, n, "Well I would but I don't know where I am going");
				npcsay(p, n, "Go to where the six heads face",
				...
				npcsay(p, n, "enjoy your stay hear",
					"May it be spiritually uplifting");
			}
		}
		switch (cID) {
			case EntranaPriest.fisherKing:
				npcsay(p, n, "The fisher king is the owner and slave of the grail");
				say(p, n, "What are the four heads?");
				//authentic from replay ended dialogue
				break;
			case EntranaPriest.sixHeads:
				npcsay(p, n, "The six  stone heads have appeared just recently in the world",
				...
				if (m == 0) {
					say(p, n, "What's a fisher king?");
					entranaPriestDialogue(p, n, EntranaPriest.fisherKing);
				} else if (m == 1) {
					say(p, n, "Ok I will go searching");
				} else if (m == 2) {
					say(p, n, "What do you mean by the whistle?");
					entranaPriestDialogue(p, n, EntranaPriest.whistle);
				} else if (m == 3) {
					say(p, n, "The point of realm crossing");
					npcsay(p, n, "The realm of the fisher king is not quite of this reality",
			...
			case EntranaPriest.whistle:
				npcsay(p, n, "You don't know about the whistles yet?",
				...
				int m1 = multi(p, n, false, //do not send over
					"What are the four heads?",
					"What's a fisher king?",
					"Ok I will go searching");
				if (m1 == 0) {
					say(p, n, "What are the six heads?");
					entranaPriestDialogue(p, n, EntranaPriest.sixHeads);
				} else if (m1 == 1) {
					say(p, n, "What's a fisher king?");
					entranaPriestDialogue(p, n, EntranaPriest.fisherKing);
