	...
	private void cartRide(Player player, Npc n) {
		...
		if (menu == 0) {
			...
			if (ifheld(player, ItemId.COINS.id(), 500)) {
				...
				mes(player, config().GAME_TICK * 2, "You Hop into the cart and the driver urges the horses on.");
				player.teleport(468, 662);
				mes(player, config().GAME_TICK * 2, "You take a taxing journey through the jungle to Brimhaven.");
				mes(player, config().GAME_TICK * 2, "You feel fatigued from the journey, but at least");
				mes(player, config().GAME_TICK * 2, "you didn't have to walk all that distance.");
			} else {
