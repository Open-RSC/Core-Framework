		...
		if (n.getID() == NpcId.JUNGLE_FORESTER.id()) {
			if (cID == -1) {
				switch (p.getQuestStage(Quests.LEGENDS_QUEST)) {
					...
					case 11:
					case -1:
						npcsay(p, n, "Hello friend, you're a long way from civilisation!");
						int opt = multi(p, n,
			...
			switch (cID) {
				case JungleForesterNPC_LegendsQuest.WHAT_DO_YOU_DO_HERE:
					npcsay(p, n, "I'm a forester, and I specialise in exotic woods. ",
				...
				case JungleForesterNPC_LegendsQuest.NATIVES_IN_THE_JUNGLE:
					npcsay(p, n, "Well, I've heard some funny sounds...",
					...
					int opt = multi(p, n,
						...
						"Ok thanks");
					if (opt == 0) {
						npcsay(p, n, "Well, I wish I could give it to you.",
						...
						int opt3 = multi(p, n,
							...
							"Ok thanks");
						if (opt3 == 0) {
							npcsay(p, n, "Well, if you have something interesting, let me have a look at it",
				...
				case JungleForesterNPC_LegendsQuest.KHARAZI_JUNGLE:
					npcsay(p, n, "Well, I've not managed it yet, ",
					...
					int option = multi(p, n,
						...
						"Ok thanks");
					if (option == 0) {
						mes(p, p.getWorld().getServer().getConfig().GAME_TICK * 2, "The forester looks very interested..");
						npcsay(p, n, "Oh, well, that sounds quite good actually...",
						...
						mes(p, 300, "The forester looks very thoughtfull");
						npcsay(p, n, "Hey, if you manage to complete it, be sure to let me take a look!",
						...
						int opt2 = multi(p, n,
							...
							"Ok thanks");
						if (opt2 == 0) {
							npcsay(p, n, "Not really, but I would say be careful, it's a dangerous place.",
								"And good luck.");
						} else if (opt2 == 1) {
							LegendsQuest_jungleForesterDialogue(p, n, JungleForesterNPC_LegendsQuest.NATIVES_IN_THE_JUNGLE);
						} else if (opt2 == 2) {
				...
				case JungleForesterNPC_LegendsQuest.MAKE_A_COPY:
					npcsay(p, n, "Many thanks friend.");
					mes(p, p.getWorld().getServer().getConfig().GAME_TICK * 2, "The Jungle Forester takes out some parchment and some charcoal.",
						"He studiously renders another copy of your map.");
					npcsay(p, n, "Many thanks friend.");
					mes(p, p.getWorld().getServer().getConfig().GAME_TICK * 2, "He takes out a strange looking object and hands it to you.");
					npcsay(p, n, "Here, I won't be needing this any longer, and it may help you.",
						"Whenever I've used it before, it attracted the attention of jungle natives.");
					give(p, ItemId.BULL_ROARER.id(), 1);
					break;
		...
		if (n.getID() == NpcId.JUNGLE_FORESTER.id()) {
			...
			switch (cID) {
				...
				case JungleForesterNPC.WHO_ARE_YOU:
					npcsay(p, n, "I'm a jungle forester,",
				...
				case JungleForesterNPC.KHARAZI_JUNGLE:
					npcsay(p, n, "Well, I've not managed it yet, ",
						...
						"Apparently he was on a mission to map the area.",
						"How foolish is that?");
					int sub_menu2 = multi(p, n, false, //do not send over
						"So someone managed to get into the Kharazi Jungle?",
						"What do you do here?",
						"Ok thanks");
					if (sub_menu2 == 0) {
						say(p, n, "So someone managed to get into the Jungle?");
						npcsay(p, n, "Yes, he said he was from some place...near the Barbarian outpost.",
							"Mentioned something about a legend ?",
							"It meant nothing to me though.");
						int sub_menu3 = multi(p, n, false, //do not send over
							"How do I get into the jungle?",
							"What do you do here?",
							"Ok thanks");
						if (sub_menu3 == 0) {
							say(p, n, "How do I get into the Kharazi jungle?");
							defaultJungleForesterDialogue(p, n, JungleForesterNPC.KHARAZI_JUNGLE);
						} else if (sub_menu3 == 1) {
							say(p, n, "What do you do here?");
							defaultJungleForesterDialogue(p, n, JungleForesterNPC.WHAT_DO_YOU_DO_HERE);
						} else if (sub_menu3 == 2) {
							say(p, n, "Ok thanks");
							defaultJungleForesterDialogue(p, n, JungleForesterNPC.OK_THANKS);
	...
	public void onUseNpc(Player p, Npc n, Item item) {
		if (n.getID() == NpcId.JUNGLE_FORESTER.id() && item.getCatalogId() == ItemId.RADIMUS_SCROLLS_COMPLETE.id()) { // the complete map.
			...
			if (p.getCarriedItems().hasCatalogID(ItemId.BULL_ROARER.id(), Optional.empty())) { // if already have the bull roarer
				npcsay(p, n, "It's a great map, thanks for letting me take a copy!",
					"It has helped me out a number of times now.");
				return;
			}
			npcsay(p, n, "*Gasp*");
			p.message("The jungle forester looks speechless.");
			npcsay(p, n, "This is very impressive!",
			...
			if (menu == 0) {
				...
				npcsay(p, n, "Well, I can offer you this?");
				mes(p, p.getWorld().getServer().getConfig().GAME_TICK * 2, "The Jungle Forester takes out a strange looking object.",
				...
				npcsay(p, n, "If you swing this above your head, it makes a strange sound.",
				...
				if (opt == 0) {
					...
					npcsay(p, n, "Very well friend, I understand, I must be on my way as well.");
