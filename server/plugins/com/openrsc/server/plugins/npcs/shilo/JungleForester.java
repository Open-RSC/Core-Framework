			...
			switch (cID) {
				...
				case JungleForesterNPC_LegendsQuest.KHARAZI_JUNGLE:
					...
					int option = multi(player, n,
						...
						"Ok thanks");
					if (option == 0) {
						mes(player, config().GAME_TICK * 2, "The forester looks very interested..");
				...
				case JungleForesterNPC_LegendsQuest.MAKE_A_COPY:
					...
					mes(player, config().GAME_TICK * 2, "The Jungle Forester takes out some parchment and some charcoal.",
						"He studiously renders another copy of your map.");
					npcsay(player, n, "Many thanks friend.");
					mes(player, config().GAME_TICK * 2, "He takes out a strange looking object and hands it to you.");
	...
	public void onUseNpc(Player player, Npc n, Item item) {
		if (n.getID() == NpcId.JUNGLE_FORESTER.id() && item.getCatalogId() == ItemId.RADIMUS_SCROLLS_COMPLETE.id()) { // the complete map.
			...
			if (menu == 0) {
				...
				mes(player, config().GAME_TICK * 2, "The Jungle Forester takes out a strange looking object.",
