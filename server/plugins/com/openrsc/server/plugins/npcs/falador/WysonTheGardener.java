...
public class WysonTheGardener implements TalkNpcTrigger {
	...
	public void onTalkNpc(Player p, Npc n) {
		...
		if (p.getQuestStage(Quests.GOBLIN_DIPLOMACY) == -1) {
			npcsay(p, n, "Hey i have heard you are looking for woad leaves");
			op = multi(p, n, "Well yes I am. Can you get some?",
				"Who told you that?");
			if (op == 1) {
				npcsay(p, n, "I can't remember now. Someone who visits this park",
					"I happen to have some woad leaves lying around",
					"Would you like to buy some?");
				int op2 = multi(p, n, "Oh yes please", "No thanks not right now");
				if (op2 == 1) return;
			}
		} else {
			npcsay(p, n, "I am the gardener round here",
				"Do you have any gardening that needs doing?");
			option = multi(p, n, "I'm looking for woad leaves", "Not right now thanks");
		}
		if (option == 0) {
			// from "Well yes I am. Can you get some?"
			if (p.getQuestStage(Quests.GOBLIN_DIPLOMACY) == -1 && op == 0) {
				npcsay(p, n, "Yes I have some somewhere");
				say(p, n, "Can I buy one please?");
				say(p, n, "Can I buy one please?");
			}
			// from "I'm looking for woad leaves"
			else if (p.getQuestStage(Quests.GOBLIN_DIPLOMACY) > 0) {
				npcsay(p, n, "Well luckily for you I may have some around here somewhere");
				say(p, n, "Can I buy one please?");
			}
			npcsay(p, n, "How much are you willing to pay?");
			int sub_option = multi(p, n, "How about 5 coins?", "How about 10 coins?",
				"How about 15 coins?", "How about 20 coins?");
			if (sub_option == 0) {
				npcsay(p, n, "No No thats far too little. Woad leaves are hard to get you know",
					"I used to have plenty but someone kept stealing them off me");
			} else if (sub_option == 1) {
				npcsay(p, n, "No No thats far to little. Woad leaves are hard to get you know",
					"I used to have plenty but someone kept stealing them off me");
				...
				if (remove(p, ItemId.COINS.id(), 20)) {
					...
					npcsay(p, n, "Here have some more you're a generous person");
