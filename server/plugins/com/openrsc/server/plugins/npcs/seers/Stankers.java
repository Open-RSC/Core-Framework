package com.openrsc.server.plugins.npcs.seers;

import com.openrsc.server.model.entity.npc.Npc;
import com.openrsc.server.model.entity.player.Player;
import com.openrsc.server.plugins.triggers.TalkNpcTrigger;

import static com.openrsc.server.plugins.Functions.*;

import com.openrsc.server.constants.ItemId;
import com.openrsc.server.constants.NpcId;

public class Stankers implements TalkNpcTrigger {

	@Override
	public void onTalkNpc(Player p, Npc n) {
		if (n.getID() == NpcId.STANKERS.id()) {
			npcsay(p, n, "Hello bold adventurer");
			int menu = multi(p, n,
				"Are these your trucks?",
				"Hello Mr Stankers");
			if (menu == 0) {
				npcsay(p, n, "Yes, I use them to transport coal over the river",
					"I will let other people use them too",
					"I'm a nice person like that",
					"Just put coal in a truck and I'll move it down to my depot over the river");
			} else if (menu == 1) {
				npcsay(p, n, "Would you like a poison chalice?");
				int subMenu = multi(p, n, false, //do not send over
					"Yes please",
					"what's a poison chalice?",
					"no thankyou");
				if (subMenu == 0) {
					say(p, n, "Yes please");
					p.message("Stankers hands you a glass of strangely coloured liquid");
					give(p, ItemId.POISON_CHALICE.id(), 1);
				} else if (subMenu == 1) {
					say(p, n, "What's a poison chalice?");
					npcsay(p, n, "It's an exciting drink I've invented",
						"I don't know what it tastes like",
						"I haven't tried it myself");
				} else if (subMenu == 2) {
					say(p, n, "No thankyou");
				}
			}
		}
	}

	@Override
	public boolean blockTalkNpc(Player p, Npc n) {
		return n.getID() == NpcId.STANKERS.id();
	}
}
