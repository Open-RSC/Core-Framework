...
public class TrawlerCatch implements OpLocTrigger {
	...
	public void onOpLoc(Player player, GameObject obj, String command) {
		if (obj.getID() == TRAWLER_CATCH) {
			mes(player, config().GAME_TICK * 3, "you search the smelly net");
				...
				for (int fishGiven = 0; fishGiven < fishCaught; fishGiven++) {
					...
					if (isFishRoll) {
						if (catchFish(81, player.getSkills().getLevel(Skills.FISHING))) {
							mes(player, config().GAME_TICK * 2, "..a manta ray!");
							...
							mes(player, config().GAME_TICK * 2, "..a sea turtle!");
							...
							mes(player, config().GAME_TICK * 2, "..a shark!");
							...
							mes(player, config().GAME_TICK * 2, "..a sword fish");
							...
							mes(player, config().GAME_TICK * 2, "..a lobster");
							...
							mes(player, config().GAME_TICK * 2, "..some tuna");
							...
							mes(player, config().GAME_TICK * 2, "..some anchovies");
							...
							mes(player, config().GAME_TICK * 2, "..a sardine");
							give(player, ItemId.RAW_SARDINE.id(), 1);
							player.incExp(Skills.FISHING, 80, false);
						} else {
							mes(player, config().GAME_TICK * 2, "..some shrimp");
						...
						if (randomJunkItem == ItemId.EDIBLE_SEAWEED.id()) { // Edible seaweed
							mes(player, config().GAME_TICK * 2, "..some seaweed");
							...
							mes(player, config().GAME_TICK * 2, "..an oyster!");
							...
							if (randomJunkItem == ItemId.BROKEN_GLASS_DIGSITE_LVL_2.id() || randomJunkItem == ItemId.BUTTONS.id()
								...
								mes(player, config().GAME_TICK * 2, "..some " + player.getWorld().getServer().getEntityHandler().getItemDef(randomJunkItem).getName());
							}
							// Old boot
							else if (randomJunkItem == ItemId.OLD_BOOT.id()) {
								mes(player, config().GAME_TICK * 2, "..an " + player.getWorld().getServer().getEntityHandler().getItemDef(randomJunkItem).getName());
							}
							// broken arrow, broken staff, Rusty sword, vase
							else {
								mes(player, config().GAME_TICK * 2, "..a " + player.getWorld().getServer().getEntityHandler().getItemDef(randomJunkItem).getName());
							}
