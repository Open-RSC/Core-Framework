	...
	public void onTalkNpc(final Player p, final Npc n) {
		if (n.getID() == NpcId.BARBARIAN_GUARD.id()) {
			...
			if (p.getCache().hasKey("barcrawl")) {
				...
				if (!p.getCarriedItems().hasCatalogID(ItemId.BARCRAWL_CARD.id(), Optional.of(false))) {
					...
					if (third == 0) {
						...
						say(p, n, "Not to bad, my barcrawl card is in my bank now");
						npcsay(p, n,
			...
			if (first == 0) {
				...
				npcsay(p, n, "Well do I look like a banker to you?");
			}
		}
	}
}
