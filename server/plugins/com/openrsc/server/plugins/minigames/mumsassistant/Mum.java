package com.openrsc.server.plugins.minigames.mumsassistant;
...
public class Mum implements TalkNpcTrigger, MiniGameInterface {
	...
	public void onTalkNpc(Player player, Npc npc) {
		...
		} else if (options.get(option).equalsIgnoreCase(whatIngredients)) {
			npcsay(player, npc, "Hehe, you can be so forgetful sometimes",
				"All I need is a tomato, a wedge of cheese, and some pizza dough",
				"Thanks again, dear!");
		} else if (options.get(option).equalsIgnoreCase(haveStuff)) {
			// Get items
		...
		} else if (options.get(option).equalsIgnoreCase(pizzaBagel)) {
			npcsay(player, npc,
				"Of course dear",
