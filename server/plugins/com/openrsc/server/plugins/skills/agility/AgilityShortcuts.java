			...
			case SHILO_VILLAGE_BRIDGE_BLOCKADE_JUMP:
				...
				if (jumpMenu == 0) {
					...
					if (succeed(p, 32)) {
						...
						p.getWorld().getServer().getGameEventHandler().add(new GameStateEvent(p.getWorld(), p, 0, "Agility Shortcut: Shilo Bridge") {
							public void init() {
								addState(0, () -> {
									...
									playerTalk(getPlayerOwner(), null, "* Ahhhhhhhhhh! *");
								...
								addState(4, () -> {
									...
									playerTalk(getPlayerOwner(), null, "* Gasp! *");
									getPlayerOwner().damage((getCurrentLevel(getPlayerOwner(), Skills.HITS) / 10));
									return nextState(2);
								});
								addState(5, () -> {
