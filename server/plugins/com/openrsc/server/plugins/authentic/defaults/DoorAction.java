	...
	public void onWallObjectAction(final GameObject obj, final Integer click,
		...
		switch (obj.getID()) {
			...
			case 68: // Crafting Guild Door
				...
				if (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.CRAFTING)) < 40) {
			...
			case 43: // Cooking Guild Door
				...
				if (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.COOKING)) < 32) {
			...
			case 146: // Magic Guild Door
				...
				if (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.MAGIC)) < 66) {
		...
		switch (obj.getID()) {
			...
			case 356: // McGrouber's Wood / Woodcutting Guild Gate (560, 472)
				...
				if (player.getY() <= 472) {
					...
					if (config().WANT_WOODCUTTING_GUILD) {
						if (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.WOODCUTTING)) < 70) {
