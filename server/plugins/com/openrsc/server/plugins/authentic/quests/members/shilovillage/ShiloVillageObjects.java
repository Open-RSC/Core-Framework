		...
		if (obj.getID() == TOMB_DOORS) {
			...
			if (command.equalsIgnoreCase("Open")) {
				...
				if (menu == 0) {
					if (!player.getCarriedItems().getEquipment().hasEquipped(ItemId.BEADS_OF_THE_DEAD.id())) {
						changeloc(obj, config().GAME_TICK * 3, 181);
						player.teleport(348, 3616);
						player.damage(getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) / 2 + 1);
						if (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) > 0) {
			...
			if (menu == 0) {
				if (player.getCarriedItems().hasCatalogID(ItemId.TATTERED_SCROLL.id(), Optional.of(false))) {
					...
					if (ShiloVillageUtils.succeed(player, 32)) {
						...
						delay(3);
						player.damage((int) (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) * 0.1D + 1));
						player.incExp(getSkillId(player.getWorld(), SkillsEnum.AGILITY), 5, true);
					}
				}
			} else if (menu == 1) {
			...
			if (m == 0) {
				...
				if (ShiloVillageUtils.succeed(player, 32)) {
					...
					player.damage((int) (getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) * 0.2D + 4));
					player.incExp(getSkillId(player.getWorld(), SkillsEnum.AGILITY), 5, true);
				} else {
	...
	public void onUseLoc(Player player, GameObject obj, Item item) {
		if (obj.getID() == TOMB_DOORS && item.getCatalogId() == ItemId.BONES.id()) {
			...
			if (!player.getCarriedItems().getEquipment().hasEquipped(ItemId.BEADS_OF_THE_DEAD.id())) {
				mes("@red@You feel invisible hands starting to choke you...");
				delay(3);
				player.damage(getCurrentLevel(player, getSkillId(player.getWorld(), SkillsEnum.HITS)) / 2);
			}
