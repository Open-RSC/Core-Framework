		...
		if (obj.getID() == TOMB_DOORS) {
			...
			if (command.equalsIgnoreCase("Open")) {
				...
				if (menu == 0) {
					if (!player.getCarriedItems().getEquipment().hasEquipped(ItemId.BEADS_OF_THE_DEAD.id())) {
						changeloc(obj, config().GAME_TICK * 3, 181);
						player.teleport(348, 3616);
						player.damage(getCurrentLevel(player, Skill.of(HITS).id()) / 2 + 1);
						if (getCurrentLevel(player, Skill.of(HITS).id()) > 0) {
			...
			if (menu == 0) {
				if (player.getCarriedItems().hasCatalogID(ItemId.TATTERED_SCROLL.id(), Optional.of(false))) {
					...
					if (ShiloVillageUtils.succeed(player, 32)) {
						...
						delay(3);
						player.damage((int) (getCurrentLevel(player, Skill.of(HITS).id()) * 0.1D + 1));
						player.incExp(Skill.of(AGILITY).id(), 5, true);
					}
				}
			} else if (menu == 1) {
			...
			if (m == 0) {
				...
				if (ShiloVillageUtils.succeed(player, 32)) {
					...
					player.damage((int) (getCurrentLevel(player, Skill.of(HITS).id()) * 0.2D + 4));
					player.incExp(Skill.of(AGILITY).id(), 5, true);
				} else {
