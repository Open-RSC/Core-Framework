...
public class Herblaw implements OpInvTrigger, UseInvTrigger {
	...
	private void handleHerbIdentify(final Item herb, Player player) {
		...
		if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < herbDef.getLevelRequired()) {
	...
	private void batchIdentify(Player player, Item herb, ItemUnIdentHerbDef herbDef) {
		...
		player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), herbDef.getExp(), true);
		delay(2);
	...
	public void onUseInv(Player player, Integer invIndex, Item item, Item usedWith) {
		...
		// Explosive compound (Digsite quest)
			...
			if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < 10) {
			...
			player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), 20, true);
			...
			player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), 25, true);
			...
			player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), 30, true);
		...
		// Blamish oil (Heroes quest)
			...
			if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < 25) {
			...
			player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), 320, true);
		...
		// Snakes weed potion (Legends quest)
			...
			if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < 45) {
	...
	private void batchPotionMaking(Player player, Item herb, ItemHerbDef herbDef, Item vial) {
		...
		if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < herbDef.getReqLevel()) {
	...
	private void batchPotionSecondary(Player player, Item unfinished, Item second, ItemHerbSecond def, AtomicReference<Item> bubbleItem) {
		if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < def.getReqLevel()) {
		...
		player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), def.getExp(), true);
		delay(2);
	...
	private void doCustomHerbSecond(Player player, int unfinishedPotId, int secondaryId) {
		...
		if (player.getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < reqLevel) {
			player.playerServerMessage(MessageType.QUEST, "You need a herblaw level of "
	...
	private void batchCustomHerbSecond(Player player, int unfinishedPotId,
		...
		player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), xp, true);
		delay(2);
	...
	private boolean makeLiquid(Player player, final Item ingredient,
		...
		if (unfinishedPotID == ItemId.UNFINISHED_OGRE_POTION.id() && ingredientID == ItemId.GROUND_BAT_BONES.id()
			...
			if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HERBLAW)) < 14) {
				player.playerServerMessage(MessageType.QUEST,
			...
			if (player.getQuestStage(Quests.WATCHTOWER) >= 0 && player.getQuestStage(Quests.WATCHTOWER) < 6) {
				...
				player.incExp(getSkillId(player.getWorld(), SkillsEnum.HERBLAW), 100, true);
			}
		}
		return false;
	}
