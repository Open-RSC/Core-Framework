	...
	public void onTalkNpc(Player player, final Npc npc) {
		...
		if (option == 0) {
			...
			for (int potion : potionIds) {
				vials += player.getCarriedItems().getInventory().countId(potion);
			}
			...
			if (heldGp >= cost) {
				...
				if (multi(player, npc, "Yes please", "No thankyou") == 0) {
					...
					delay(3);
					npcsay(player, npc, "There you are", "Enjoy!");
				}
			} else {
				npcsay(player, npc,
			...
			int vials = player.getCarriedItems().getInventory().countId(ItemId.EMPTY_VIAL.id());
			...
			int heldGp = player.getCarriedItems().getInventory().countId(ItemId.COINS.id());
			npcsay(player, npc,
				"Since you're holding " + vials + " vials, that will cost " + cost + " gold");
			if (heldGp >= cost) {
				...
				if (multi(player, npc, "Yes please", "No thank you") == 0) {
					...
					// Give vials
					player.getCarriedItems().getInventory().add(new Item(ItemId.VIAL.id(), totalRemoved, true));
