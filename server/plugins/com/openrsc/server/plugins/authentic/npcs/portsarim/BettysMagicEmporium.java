...
public final class BettysMagicEmporium extends AbstractShop {
	...
	public void onTalkNpc(final Player player, final Npc npc) {
		if (npc.getID() == NpcId.BETTY.id()) {
			...
			if (config().MICE_TO_MEET_YOU_EVENT) {
				if (opt == 2 && (questStage == 4 || questStage == 5)) {
					miceToMeetYou(player, npc);
				} else if (opt == 2 && questStage == 6) {
	...
	private void miceToMeetYou(final Player player, final Npc npc) {
		...
		if (ifvar("mice_to_meet_you", 4)) {
			...
			npcsay("It's quite a simple spell really",
				"You bring me the ingredients, and I'll teach it to you",
				"All I'll need is 10 body runes,",
				"1 eye of a newt,",
				"And you'll need to be wearing a wizard hat");
			...
			while (option != 0) {
				...
				setvar("mice_to_meet_you", 5);
			}
		} else if (ifvar("mice_to_meet_you", 5)) {
			...
			if (multi("Alright then",
					"Hang on, don't you sell all those things?") == 1) {
				npcsay("Ah, would you look at that?",
					"It seems that I do",
					"How convenient for you");
				say("Can't we just use what you already have in stock?");
	...
	private void miceSpell(final Player player, final Npc npc) {
		...
		npcsay("And repeat after me",
			"Custodire");
		option = multi("Custodire", "Custodian", "Stop!");
		if (option == 2 || option == -1) return;
		else if (option == 1) {
			messUp();
			return;
		...
		delay(3);
		npcsay("Brassica!");
		option = multi("Brassica", "Cabbage", "Stop!");
		if (option == 2 || option == -1) return;
		else if (option == 1) {
			messUp();
			return;
		...
		npcsay("Eak is now protected from Death's magic",
			...
			"If you find anything, go to Aggie in Draynor",
			"I don't think I'll be of any more help");
		setvar("mice_to_meet_you", 7);
	}
	...
	private boolean isWearingHat(final Player player) {
		int[] hatIds = {
			ItemId.A_BLUE_WIZARDS_HAT.id(),
			ItemId.BLUE_WIZARDSHAT.id(),
			ItemId.BLACK_WIZARDSHAT.id()
		...
		for (int hatId : hatIds) {
			if (player.getCarriedItems().getEquipment().hasEquipped(hatId)) return true;
		}
