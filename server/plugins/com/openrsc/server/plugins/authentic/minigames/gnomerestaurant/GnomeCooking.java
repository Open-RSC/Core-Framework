...
public class GnomeCooking implements OpInvTrigger, UseLocTrigger {
	...
	private void handleGnomeCooking(final Item item, Player player, final GameObject object) {
		...
		if (player.getCarriedItems().remove(item) > -1) {
			mes(gc.messages[0]);
			delay(5);
			if (!burnFood(player, item.getCatalogId(), player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.COOKING)))) {
				...
				if (recipeSuccess) {
					player.incExp(getSkillId(player.getWorld(), SkillsEnum.COOKING), gc.experience, true);
	...
	private boolean mouldDough(Item item, Player player) {
		...
		if (menu != -1) {
			player.setOption(-1);
			if (menu == 0) {
				if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.COOKING)) < 25) {
				...
				if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.COOKING)) < 30) {
				...
				if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.COOKING)) < 15) {
			...
			player.incExp(getSkillId(player.getWorld(), SkillsEnum.COOKING), 100, true);
		}
		return true;
