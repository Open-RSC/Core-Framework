...
public class SheepShearer implements QuestInterface, TalkNpcTrigger {
	...
	private void fredDialogue(Player p, Npc n, int cID) {
			...
			switch (p.getQuestStage(this)) {
				case 0:
					...
					if (choice == 0) {
						...
						if (choice1 == 0) {
							npcsay(p, n, "Ok I'll see you when you have some wool");
							...
							npcsay(p, n, "I wouldn't worry about it",
					...
					break;
				case 1:
					npcsay(p, n, "How are you doing getting those balls of wool?");
					int totalWool = 0;
					...
					if (woolCount > 0) {
						...
						if (totalWool >= 20) {
							...
							npcsay(p, n, "I guess I'd better pay you then");
							...
							say(p, n, "That's all I've got so far");
							p.getCache().set("sheep_shearer_wool_count", totalWool);
							npcsay(p, n, "I need more before I can pay you");
							say(p, n, "Ok I'll work on it");
						}
					} else if (p.getCarriedItems().hasCatalogID(ItemId.WOOL.id(), Optional.of(false))) {
						say(p, n, "Well I've got some wool",
							"I've not managed to make it into a ball though");
						npcsay(p, n, "Well go find a spinning wheel then",
							"And get spinning");
					} else {
						say(p, n, "I haven't got any at the moment");
						npcsay(p, n, "Ah well at least you haven't been eaten");
					}
					break;
				case -1:
					npcsay(p, n, "What are you doing on my land?");
		...
		switch (cID) {
			case Fred.KILL:
				npcsay(p, n, "What on my land?",
					"Leave my livestock alone you scoundrel");
				break;
			case Fred.LOST:
				npcsay(p, n, "How can you be lost?",
