...
public class LegendsQuestMapJungle implements OpInvTrigger {
		...
		if (item.getCatalogId() == ItemId.RADIMUS_SCROLLS_COMPLETE.id()) {
			...
			if (menu == 0) {
				...
				if (!JUNGLE_WEST_AREA(player) && !JUNGLE_MIDDLE_AREA(player) && !JUNGLE_EAST_AREA(player)) {
					...
					if (rnd == 0) {
						mes(config().GAME_TICK * 2, "You're not even in the Kharazi Jungle yet.");
						mes(config().GAME_TICK * 2, "You need to get to the Southern end of Karamja ");
						mes(config().GAME_TICK * 2, "before you can start mapping.");
					}
					else {
						mes(config().GAME_TICK * 3, "You prepare to start mapping this area...");
						mes(config().GAME_TICK * 2, "This doesn't look like the Kharazi Jungle! ");
						mes(config().GAME_TICK * 2, "You need to go to the very southern end of the Island of Karamja !");
					}
					return;
				}
				mes(config().GAME_TICK * 3, "You prepare to start mapping this area...");
				if (player.getCache().hasKey("JUNGLE_EAST") && JUNGLE_EAST_AREA(player)) {
					mes(config().GAME_TICK * 2, "You have already completed this part of the map.");
				...
				if (!player.getCarriedItems().hasCatalogID(ItemId.PAPYRUS.id(), Optional.of(false))
					...
					mes(config().GAME_TICK * 2, "You'll need some papyrus and charcoal to complete this map.");
					canMap = false;
					...
					mes(config().GAME_TICK * 2, "You'll need some charcoal to complete this map.");
					canMap = false;
				} else if (!player.getCarriedItems().hasCatalogID(ItemId.PAPYRUS.id(), Optional.of(false))
					&& player.getCarriedItems().hasCatalogID(ItemId.A_LUMP_OF_CHARCOAL.id(), Optional.of(false))) { // has charcoal but no papyrus
					mes(config().GAME_TICK * 2, "You'll need some additional Papyrus to complete this map.");
					canMap = false;
				}
	...
	private void checkMapComplete(Player player) {
		if (!player.getCache().hasKey("JUNGLE_EAST")) {
			mes(config().GAME_TICK * 2, "@red@You have yet to map the eastern part of the Kharazi Jungle");
		} else {
			mes(config().GAME_TICK * 2, "@gre@Eastern area of the Kharazi Jungle - *** Completed ***");
		}
		if (!player.getCache().hasKey("JUNGLE_MIDDLE")) {
			mes(config().GAME_TICK * 2, "@red@You have yet to map the mid - part of the Kharazi Jungle.");
		} else {
			mes(config().GAME_TICK * 2, "@gre@Middle area of the Kharazi Jungle- *** Completed ***");
		}
		if (!player.getCache().hasKey("JUNGLE_WEST")) {
			mes(config().GAME_TICK * 2, "@red@You have yet to map the Western part of the Kharazi Jungle.");
		} else {
			mes(config().GAME_TICK * 2, "@gre@Western part of the Kharazi Jungle- *** Completed ***");
		}
	}
	...
	private void mapArea(Player player) {
		...
		if (random <= 29) { // 30% succeed.
			...
			mes(config().GAME_TICK * 2, "You neatly add a new section to your map.");
			...
			if (player.getCache().hasKey("JUNGLE_EAST") && player.getCache().hasKey("JUNGLE_MIDDLE") && player.getCache().hasKey("JUNGLE_WEST")) {
				mes(config().GAME_TICK * 2, "Well done !",
					"You have completed mapping the Kharazai jungle on the southern end of Karamja,");
				mes(config().GAME_TICK * 3, "Grand Vizier Erkle will be pleased.");
				...
				mes(config().GAME_TICK * 3, "You still have some sections of the map to complete.");
				checkMapComplete(player);
			}
