...
public class LegendsQuestInvAction implements OpInvTrigger, UseInvTrigger {
	...
	public void onOpInv(Player player, Integer invIndex, Item item, String command) {
		if (item.getCatalogId() == ItemId.GILDED_TOTEM_POLE.id()) {
			mes(player, config().GAME_TICK * 2, "This totem pole is utterly awe inspiring.",
			...
			if (n != null && player.getQuestStage(Quests.LEGENDS_QUEST) == 7) {
				...
				mes(player, n, config().GAME_TICK * 2, "A bright, holy light streams out from the paper spell.");
				...
				mes(player, n, config().GAME_TICK * 2, "The spirit lets out an unearthly, blood curdling scream...");
				...
				if (second_nezikchened != null) {
					mes(player, second_nezikchened, config().GAME_TICK, "The spell seems to weaken the Demon.");
					...
					if (player.getSkills().getLevel(Skills.PRAYER) - newPray < 30) {
						mes(player, config().GAME_TICK * 2, "A sense of fear comes over you ",
							"You feel a sense of loss...");
					} else {
						mes(player, config().GAME_TICK * 2, "An intense sense of fear comes over you ",
							"You feel a great sense of loss...");
					}
					player.getSkills().setLevel(Skills.PRAYER, newPray);
					if (player.getCache().hasKey("ran_from_2nd_nezi")) {
						delay(7000);
						mes(player, config().GAME_TICK * 2, "The Demon takes out a dark dagger and throws it at you...");
						if (DataConversions.random(0, 1) == 1) {
							mes(player, config().GAME_TICK * 2, "The dagger hits you with an agonising blow...");
							player.damage(14);
						} else {
							mes(player, config().GAME_TICK, "But you neatly manage to dodge the attack.");
						}
					}
				}

			} else {
				mes(player, config().GAME_TICK, "There is no suitable candidate to cast this spell on.");
			}
		}
		else if (item.getCatalogId() == ItemId.A_RED_CRYSTAL.id() && !config().WANT_PETS) {
			mes(player, config().GAME_TICK * 2, "As the crystal touches your hands a voice inside of your head says..",
				"@gre@Voice in head: Bring life to the dragons eye.");
		}
		else if (item.getCatalogId() == ItemId.GERMINATED_YOMMI_TREE_SEED.id()) {
			mes(player, config().GAME_TICK * 2, "These seeds have been germinated in pure water...");
			...
			int page = multi(player,
				...
				"Enchanto...");
			if (page == 0) {
				mes(player, config().GAME_TICK * 2, "You read the section entitled Arcana...");
				...
				mes(player, config().GAME_TICK * 2, "You read the section entitled Instructo...");
				...
				mes(player, config().GAME_TICK * 2, "You read the section entitled Defeati...");
				...
				mes(player, config().GAME_TICK * 2, "You read the section entitled Enchanto...",
			...
			mes(player, config().GAME_TICK * 2, "You look at the rough sketch that Gujuo gave you.");
			player.message("It looks like a picture of a bowl...");
		}
		else if (item.getCatalogId() == ItemId.SHAMANS_TOME.id()) {
			mes(player, config().GAME_TICK * 2, "You read the ancient shamans tome.");
			mes(player, config().GAME_TICK * 6, "It is written in a strange sort of language but you manage a rough translation.");
	...
	public void onUseInv(Player player, Integer invIndex, Item item1, Item item2) {
		if (compareItemsIds(item1, item2, ItemId.YOMMI_TREE_SEED.id(), ItemId.BLESSED_GOLDEN_BOWL_WITH_PURE_WATER.id())) {
			...
			mes(player, config().GAME_TICK * 2, "The pure water in the golden bowl has run out...");
