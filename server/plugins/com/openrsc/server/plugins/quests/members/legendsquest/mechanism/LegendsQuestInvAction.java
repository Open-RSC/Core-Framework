	...
	public void onOpInv(Item item, Player player, String command) {
		if (item.getCatalogId() == ItemId.GILDED_TOTEM_POLE.id()) {
			...
			if (n != null && player.getQuestStage(Quests.LEGENDS_QUEST) == 7) {
				...
				mes(player, n, player.getWorld().getServer().getConfig().GAME_TICK * 2, "The spirit lets out an unearthly, blood curdling scream...");
				...
				mes(player, player.getWorld().getServer().getConfig().GAME_TICK, "There is no suitable candidate to cast this spell on.");
			}
		}
		else if (item.getCatalogId() == ItemId.A_RED_CRYSTAL.id() && !player.getWorld().getServer().getConfig().WANT_PETS) {
			mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "As the crystal touches your hands a voice inside of your head says..",
				"@gre@Voice in head: Bring life to the dragons eye.");
		}
		else if (item.getCatalogId() == ItemId.GERMINATED_YOMMI_TREE_SEED.id()) {
			mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "These seeds have been germinated in pure water...");
			...
			int page = multi(player,
				...
				"Enchanto...");
			if (page == 0) {
				mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "You read the section entitled Arcana...");
				...
				mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "You read the section entitled Instructo...");
				...
				mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "You read the section entitled Defeati...");
				...
				mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "You read the section entitled Enchanto...",
			...
			mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "You look at the rough sketch that Gujuo gave you.");
			player.message("It looks like a picture of a bowl...");
		}
		else if (item.getCatalogId() == ItemId.SHAMANS_TOME.id()) {
			mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 2, "You read the ancient shamans tome.");
			mes(player, player.getWorld().getServer().getConfig().GAME_TICK * 6, "It is written in a strange sort of language but you manage a rough translation.");
