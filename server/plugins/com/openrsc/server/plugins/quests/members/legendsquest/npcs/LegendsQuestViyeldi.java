...
public class LegendsQuestViyeldi implements TalkNpcTrigger, TakeObjTrigger, AttackNpcTrigger, SpellNpcTrigger, PlayerRangeNpcTrigger {
	...
	public void onTalkNpc(Player player, Npc n) {
		if (n.getID() == NpcId.VIYELDI.id()) {
			switch (player.getQuestStage(Quests.LEGENDS_QUEST)) {
				case 7:
					mes(player, n, config().GAME_TICK * 2, "The headless, spirit of Viyeldi animates and walks towards you.");
					if (!player.getCache().hasKey("killed_viyeldi")) {
						mes(player, n, config().GAME_TICK * 2, "And starts talking to you in a shrill, excited voice...");
						...
						mes(player, n, config().GAME_TICK * 2, "The spirit wavers slightly and then stands proud...");
	...
	public void onTakeObj(Player player, GroundItem i) {
		if (i.getID() == ItemId.A_BLUE_WIZARDS_HAT.id() && i.getX() == 426 && i.getY() == 3708) {
			...
			mes(player, config().GAME_TICK * 2, "Your hand passes through the hat as if it wasn't there.");
			if (player.getQuestStage(Quests.LEGENDS_QUEST) >= 8) {
				return;
			}
			player.teleport(i.getX(), i.getY() - 1);
			mes(player, config().GAME_TICK * 2, "Instantly the clothes begin to animate and then walk towards you.");
	...
	private void attackViyeldi(Player player, Npc n) {
		if (n.getID() == NpcId.VIYELDI.id()) {
			if (!player.getCarriedItems().getEquipment().hasEquipped(ItemId.DARK_DAGGER.id())) {
				mes(player, n, config().GAME_TICK * 2, "Your attack passes straight through Viyeldi.");
				...
				mes(player, n, config().GAME_TICK * 2, "You thrust the Dark Dagger at Viyeldi...");
				npcsay(player, n, "So, you have fallen for the foul one's trick...");
				mes(player, n, config().GAME_TICK * 2, "You hit Viyeldi squarely with the Dagger .");
				npcsay(player, n, "AhhhhhhhhHH! The Pain!");
				mes(player, n, config().GAME_TICK * 2, "You see a flash as something travels from Viyeldi into the dagger.");
