...
public class MurderMystery implements QuestInterface, TalkNpcTrigger,
	...
	private static void sinclairSuspectDialogue(Player p, Npc n) {
		say(p, n, "I'm here to help the guards with their investigation");
		if (n.getID() == NpcId.CAROL_SINCLAIR.id()) {
			npcsay(p, n, "Well, ask what you want to know then");
		} else if (n.getID() == NpcId.ELIZABETH_SINCLAIR.id()) {
			npcsay(p, n, "What's so important you need to bother me with then?");
		} else if (n.getID() == NpcId.ANNA_SINCLAIR.id()) {
			npcsay(p, n, "Oh really? what do you want to know then?");
		} else if (n.getID() == NpcId.FRANK_SINCLAIR.id()) {
			npcsay(p, n, "Good for you. Now what do you want?",
				"And can you spare me any money? I'm a little short...");
		} else if (n.getID() == NpcId.BOB_SINCLAIR.id()) {
			npcsay(p, n, "I suppose I had better talk to you then.");
		} else if (n.getID() == NpcId.DAVID_SINCLAIR.id()) {
			npcsay(p, n, "And? Make this quick, I have better things to",
				"do than be interrogated by halfwits all day");
		}
		int menu;
		if (p.getCache().hasKey("poison_opt") && p.getCache().hasKey("thread")) {
			menu = multi(p, n, "Who do you think was responsible?", "Where were you when the murder happened?", "Do you recognise this thread?", "Why did you buy poison the other day?");
		} else if (p.getCache().hasKey("poison_opt") && !p.getCache().hasKey("thread")) {
			menu = multi(p, n, "Who do you think was responsible?", "Where were you when the murder happened?", "Why did you buy poison the other day?");
		} else if (p.getCache().hasKey("thread")) {
			menu = multi(p, n, "Who do you think was responsible?", "Where were you when the murder happened?", "Do you recognise this thread?");
		} else {
			menu = multi(p, n, "Who do you think was responsible?", "Where were you when the murder happened?");
		}


		if (menu == 0) {
			if (n.getID() == NpcId.CAROL_SINCLAIR.id()) {
				npcsay(p, n, "I don't know. I think its very convenient",
					"that you have arrived here so soon after it happened.",
					"Maybe it was you");
			} else if (n.getID() == NpcId.ELIZABETH_SINCLAIR.id()) {
				npcsay(p, n, "Could have been anyone. The old man was an",
					"idiot. Hes been asking for it for years.");
			} else if (n.getID() == NpcId.ANNA_SINCLAIR.id()) {
				npcsay(p, n, "It was clearly an intruder.");
				say(p, n, "Well, I don't think it was");
				npcsay(p, n, "It was one of our lazy servants then");
			} else if (n.getID() == NpcId.FRANK_SINCLAIR.id()) {
				...
				npcsay(p, n, "I don't really care as long as noone thinks its me",
					"Maybe that strange poison seller who headed towards the seers village.");
			} else if (n.getID() == NpcId.DAVID_SINCLAIR.id()) {
				npcsay(p, n, "I don't really know or care",
			...
			if (n.getID() == NpcId.CAROL_SINCLAIR.id()) {
				npcsay(p, n, "Why? Are you accusing me of something?",
					"You seem to have a very high opinion of yourself",
					"I was in my room if you must know, alone.");
			} else if (n.getID() == NpcId.ELIZABETH_SINCLAIR.id()) {
				npcsay(p, n, "I was out");
				say(p, n, "Care to be any more specific?");
				npcsay(p, n, "not really. I don't have to justify myself to the likes of you.",
					"I know the king personally you know. Now are we finished here?");
			} else if (n.getID() == NpcId.ANNA_SINCLAIR.id()) {
				npcsay(p, n, "in the library. Noone else was there so",
					"you'll just have to take my word for it");
			} else if (n.getID() == NpcId.FRANK_SINCLAIR.id()) {
				npcsay(p, n, "I don't know, somewhere around here probably.",
					"Could you spare me a few coins?",
					"I'll be able to pay you double tomorrow",
					"its just theres this poker night tonight in town...");
			} else if (n.getID() == NpcId.BOB_SINCLAIR.id()) {
				npcsay(p, n, "I was walking by myself in the garden.");
				say(p, n, "And can anyone vouch for that?");
				npcsay(p, n, "No. But I was.");
			} else if (n.getID() == NpcId.DAVID_SINCLAIR.id()) {
				npcsay(p, n, "that is none of your business.",
			...
			if (n.getID() == NpcId.CAROL_SINCLAIR.id() && !p.getCarriedItems().hasCatalogID(ItemId.THREAD_RED.id())) {
				...
				npcsay(p, n, "Its some thread. Sorry, do you have a point here?",
				...
				npcsay(p, n, "Its some red thread... it kind of looks like the",
				...
				p.message("You show her the thread from the study window");
				npcsay(p, n, "Its some thread. You're not very good",
					"at this whole investigation thing are you?");
			} else if (n.getID() == NpcId.ELIZABETH_SINCLAIR.id() && p.getCarriedItems().hasCatalogID(ItemId.THREAD_BLUE.id())) {
				p.message("You show her the thread from the study window");
				npcsay(p, n, "Looks like a Blue thread to me.",
					" If you can't work that out for yourself I",
					"don't hold much hope of you solving this crime.");
				say(p, n, "It looks a lot like the material your trousers",
					"are made of doesn't it?");
				npcsay(p, n, "I suppose it does. So what?");
				...
				npcsay(p, n, "Not really, no. Thread is fairly common");
			} else if (n.getID() == NpcId.ANNA_SINCLAIR.id() && p.getCarriedItems().hasCatalogID(ItemId.THREAD_GREEN.id())) {
				p.message("You show Anna the thread from the study");
				npcsay(p, n, "Its some Green thread. Its not exactly uncommon is it?",
				...
				npcsay(p, n, "It looks like thread to me, but I'm not exactly",
					"an expert. Is it worth something?",
					"Can I have it? Actually, can you spare me a few gold?");
			} else if (n.getID() == NpcId.FRANK_SINCLAIR.id() && p.getCarriedItems().hasCatalogID(ItemId.THREAD_BLUE.id())) {
				p.message("Frank examines the thread from the crime scene");
				npcsay(p, n, "it kind of looks like the same material as",
				...
				npcsay(p, n, "Its some thread. great clue. No, really.");
			} else if (n.getID() == NpcId.BOB_SINCLAIR.id() && p.getCarriedItems().hasCatalogID(ItemId.THREAD_RED.id())) {
				p.message("you show him the thread you discovered");
				npcsay(p, n, "Its some red thread. I suppose you think",
				...
				npcsay(p, n, "No. Can I go yet? your face irritates me.");
			} else if (n.getID() == NpcId.DAVID_SINCLAIR.id() && p.getCarriedItems().hasCatalogID(ItemId.THREAD_GREEN.id())) {
				p.message("You show him the thread you found on the study window");
				npcsay(p, n, "Its some Green thread, like my trousers are made of.",
			...
			if (n.getID() == NpcId.CAROL_SINCLAIR.id()) {
				npcsay(p, n, "I don't see what on earth it has to",
				...
				npcsay(p, n, "there was a nest of mosquitos under the fountain",
				...
				say(p, n, "I hate mosquitos");
				npcsay(p, n, "Doesn't everyone?");
				if (p.getCache().hasKey("murder_eliz")) {
					p.getCache().store("p_eliza", true);
					p.getCache().store("poison_opt2", true);
				}
			} else if (n.getID() == NpcId.ANNA_SINCLAIR.id()) {
				npcsay(p, n, "That useless Gardener Stanford has let his",
				...
				npcsay(p, n, "Would you like to buy some? I'm kind of strapped",
				...
				npcsay(p, n, "what's it to you anyway?",
				...
				npcsay(p, n, "There was a nest of spiders upstairs between the",
	...
	private void whoYouSuspect(Player p, Npc n) {
		...
		npcsay(p, n, "Great work, show me the evidence",
			"and we'll take them to the dungeons",
			"you *DO* have evidence of their crime, right?");
		say(p, n, "uh....");
		npcsay(p, n, "tch. You wouldn't last a day in the guards",
	...
	public void onTalkNpc(Player p, Npc n) {
		...
		if (n.getID() == NpcId.GUARD_SINCLAIR_MANSION.id()) {
			switch (p.getQuestStage(this)) {
				case 0:
					say(p, n, "What's going on here?");
					npcsay(p, n, "Oh, its terrible.",
					...
					if (menu == 0) {
						...
						say(p, n, "What should I be doing to help?");
						npcsay(p, n, "Look around and investigate who might be responsible",
						...
						npcsay(p, n, "get lost then, this is private property.",
				...
				case 1:
					...
					if (opt == 0) {
						...
						npcsay(p, n, "well its all very mysterious.",
						...
						if (!p.getCache().hasKey("thread") && !p.getCache().hasKey("evidence") && !p.getCache().hasKey("culprit")) {
							npcsay(p, n, "Really? That was quick work! Who?");
							int variableD = multi(p, n, "It was an intruder!", "the butler did it!", "It was one of the servants", "It was one of his family");
							if (variableD == 0) {
								npcsay(p, n, "Thats what we were thinking too.",
								...
								npcsay(p, n, "I hope you have proof to that effect.",
								...
								delay(1500);
								npcsay(p, n, "although having said that",
									"the butler is kind of shifty looking...");
							} else if (variableD == 2) {
								npcsay(p, n, "Oh really? Which one?");
								int variableA = multi(p, n,
									"It was one of the women",
									"It was one of the men");
								if (variableA == 0) {
									npcsay(p, n, "Oh really? Which one?");
									//do not send over
									int variableB = multi(p, n, false,
								...
								npcsay(p, n, "Oh really? Which one?");
								int family = multi(p, n,
									"It was one of the women",
									"It was one of the men");
								if (family == 0) {
									npcsay(p, n, "Oh really? Which one?");
									//do not send over
									int variableI = multi(p, n, false,
										"I know it was Anna",
						...
						//3 pieces
							...
							say(p, n, "I have conclusive Proof who the killer was");
							npcsay(p, n, "You do? thats excellent work. Lets hear it then");
							say(p, n, "I don't think it was an intruder, and I don't think Lord",
								"Sinclair was killed by being stabbed.");
							npcsay(p, n, "hmmm? really? why not?");
							say(p, n, "nobody heard the guard dog barking, which it would have if",
							...
							npcsay(p, n, "Yes, that makes sense. But who did do it then?");
							if (p.getCache().hasKey("murder_david")) {
							...
							npcsay(p, n, "Yes, I'd have to agree with that... but we need more evidence");
							if (p.getCache().hasKey("murder_david")) {
							...
							npcsay(p, n, "Excellent work - have you considered a career as a detective?",
					...
					break;
				case -1:
					npcsay(p, n, "Excellent work on solving the murder",
		...
		/** START SINCLAIRS **/
			...
			if (p.getQuestStage(this) == 0) {
				...
				npcsay(p, n, "Apparently you aren't as stupid as you look");
			} else {
		...
		/** START OTHER NPCS **/
			...
			if (p.getQuestStage(this) == 0) {
				npcsay(p, n, "This is private property! Please leave!");
			} else if (p.getQuestStage(this) == -1) {
				npcsay(p, n, "Thank you for all your help in solving the murder");
			} else {
			...
			if (p.getQuestStage(this) == 0) {
				npcsay(p, n, "I'm far too upset to talk to random people right now");
			} else if (p.getQuestStage(this) == -1) {
				npcsay(p, n, "Thank you for all your help in solving the murder");
			} else {
				otherSuspectDialogue(p, n);
			...
			if (p.getQuestStage(this) == 0) {
				npcsay(p, n, "Have you no shame? we are all grieving at the moment");
			} else if (p.getQuestStage(this) == -1) {
				npcsay(p, n, "Thank you for all your help in solving the murder");
			} else {
				otherSuspectDialogue(p, n);
			...
			if (p.getQuestStage(this) == 0) {
				npcsay(p, n, "The Guards told me not to talk to anyone");
			} else if (p.getQuestStage(this) == -1) {
				npcsay(p, n, "Thank you for all your help in solving the murder");
			} else {
				otherSuspectDialogue(p, n);
			...
			if (p.getQuestStage(this) == 0) {
				npcsay(p, n, "Theres some kind of commotion up at the Sinclair place",
					"I hear. Not surprising all things considered");
			} else if (p.getQuestStage(this) == -1) {
				npcsay(p, n, "I heard you solved the murder",
					"Was I of any help to you at all?");
			} else {
				say(p, n, "I'm investigating the murder up at the Sinclair place");
				npcsay(p, n, "Murder is it?",
				...
				if (menu == 0) {
					npcsay(p, n, "Well, what do you want to know?");
					int menu2 = multi(p, n,
						"Tell me about Lord Sinclair",
						"what can you tell me about his sons?",
						"what can you tell me about his daughters?");
					if (menu2 == 0) {
						npcsay(p, n, "Old Lord Sinclair was a great man with a lot of",
							"respect in these parts. More than his worthless",
							"children have anyway");
						say(p, n, "His children? They have something to gain by his death?");
						npcsay(p, n, "yes. you could say that. not that im one to gossip");
					} else if (menu2 == 1) {
						npcsay(p, n, "His sons eh? They all have their own skeletons",
						...
						int menu3 = multi(p, n,
							...
							"Tell me about Frank");
						if (menu3 == 0) {
							npcsay(p, n, "Bob is an odd character indeed...",
							...
							npcsay(p, n, "I'm not one to talk ill of people behind their back",
						...
						npcsay(p, n, "His daughters eh? They're all nasty pieces of work",
						...
						if (menu4 == 0) {
							...
							npcsay(p, n, "Oh Carol... she is such a fool",
							...
							npcsay(p, n, "Elizabeth? Elizabeth has a strange problem",
					...
					npcsay(p, n, "well, I guess it could have been an intruder",
					...
					npcsay(p, n, "Well, they used to live in the big castle",
					...
					switch (hint) {
						case 0:
							...
							break;
						case 1:
							npcsay(p, n, "Well I don't know how much help this is",
							...
							break;
						case 2:
							npcsay(p, n, "Well, this might be of some help to you",
							...
							break;
						case 3:
							npcsay(p, n, "I don't know how much help this is to you",
							...
							break;
						case 4:
							npcsay(p, n, "My father used to be in the guard.",
			...
			if (p.getQuestStage(this) == 0) {
				npcsay(p, n, "I have no interest in talking to gawkers");
			} else if (p.getQuestStage(this) == -1) {
				npcsay(p, n, "Thank you for all your help in solving the murder");
			} else {
				otherSuspectDialogue(p, n);
			...
			switch (p.getQuestStage(this)) {
				case 0:
					say(p, n, "Hi.");
					npcsay(p, n, "I'm afraid I'm all sold out of poison at the moment.",
						"People know a bargain when they see it!");
					break;
				case 1:
					say(p, n, "I'm investigating the murder at the Sinclair house.");
					npcsay(p, n, "There was a murder at the Sinclair House???",
					...
					if (menu == 0) {
						npcsay(p, n, "Aaaaah... a miracle of modern apothecarys, this exclusive",
						...
						say(p, n, "uh... no, its ok");
					} else if (menu == 1) {
						npcsay(p, n, "Well, Peter Potters Patented Multi Purpose Poison",
						...
						npcsay(p, n, "I'm afraid I am totally out of stock at the moment",
						...
						npcsay(p, n, "hmmm... yes, that smells exactly like my",
						...
						say(p, n, "Perhaps someone else put it in his wine?");
						npcsay(p, n, "yes... I suppose that could have happened...");
					}
					break;
			}
		}
	...
	private void otherSuspectDialogue(Player p, Npc n) {
		...
		if (menu == 0) {
			if (n.getID() == NpcId.HOBBES_THE_BUTLER.id()) {
				npcsay(p, n, "Well, in my considered opinion it must be",
				...
				npcsay(p, n, "It was Anna. She is seriously unbalanced.",
				...
				npcsay(p, n, "honestly? I think it was Carol.",
				...
				npcsay(p, n, "Oh... I really couldn't say.",
			...
			if (n.getID() == NpcId.HOBBES_THE_BUTLER.id()) {
				npcsay(p, n, "I was assisting the cook with the evening meal",
					"I gave Mary His Lordships dinner, and sent her",
					"to take it to him, then heard the scream as she",
					"found the body.");
			} else if (n.getID() == NpcId.STANFORD_THE_GARDENER.id()) {
				npcsay(p, n, "Right here, by my little shed.",
					"Its very cosy to sit and think in");
			} else if (n.getID() == NpcId.PIERRE_THE_DOG_HANDLER.id()) {
				npcsay(p, n, "I was in town at the inn. When I got back",
					"The house was swarming with guards who told",
					"me what had happened. Sorry.");
			} else if (n.getID() == NpcId.LOUISA_THE_COOK.id()) {
				npcsay(p, n, "I was right here with Hobbes and Mary.",
					"You can't suspect me surely!");
			} else if (n.getID() == NpcId.MARY_THE_MAID.id()) {
				npcsay(p, n, "I was with hobbes and Louisa in the Kitchen",
			...
			if (n.getID() == NpcId.HOBBES_THE_BUTLER.id()) {
				npcsay(p, n, "how do you mean suspicious?");
				say(p, n, "Any sounds of a struggle with Lord Sinclair?");
				npcsay(p, n, "No, I definitely didn't hear anything like that.");
				say(p, n, "How about the guard dog barking at all?");
				npcsay(p, n, "You know, now you come to mention it",
					"I don't believe I did. I suppose that is",
					"Proof enough that it could not have been an",
					"intruder who is responsible.");
			} else if (n.getID() == NpcId.STANFORD_THE_GARDENER.id()) {
				npcsay(p, n, "Not that I remember.");
				say(p, n, "So no sounds of a struggle between Lord Sinclair and an intruder?");
				npcsay(p, n, "Not to the best of my recollection");
				say(p, n, "How about the guard dog barking?");
				npcsay(p, n, "Not that I can recall");
			} else if (n.getID() == NpcId.PIERRE_THE_DOG_HANDLER.id()) {
				npcsay(p, n, "well, like what?");
				say(p, n, "Any sounds of a struggle with Lord Sinclair?");
				npcsay(p, n, "No, I don't remember hearing anything like that.");
				say(p, n, "How about the guard dog barking at all?");
				npcsay(p, n, "I hear him bark all the time.",
				...
				npcsay(p, n, "suspicious? what do you mean suspicious?");
				say(p, n, "Any sounds of a struggle with an intruder for example?");
				npcsay(p, n, "No, I'm sure I don't recall any such thing.");
				say(p, n, "How about the guard dog barking at an intruder?");
				npcsay(p, n, "No, I didn't.",
					"If you don't have anything else to ask can",
					"You go and leave me alone now? I have a lot",
					"Of cooking to do for this evening.");
			} else if (n.getID() == NpcId.MARY_THE_MAID.id()) {
				npcsay(p, n, "I don't really remember hearing anything out of the ordinary");
				say(p, n, "no sounds of a struggle then?");
				npcsay(p, n, "No, I don't remember hearing anything like that.");
				say(p, n, "How about the guard dog barking?");
				npcsay(p, n, "Oh that horrible dog is always barking at nothing",
					"but I don't think I did...");
			} else if (n.getID() == NpcId.DONOVAN_THE_HANDYMAN.id()) {
				npcsay(p, n, "hmmm..... No, I didn't, but I sleep very soundly at night.");
				say(p, n, "So you didn't hear any sounds of a struggle or any",
					"barking from the guard dog next to his study window?");
				npcsay(p, n, "Now you mention it, no. it is odd I didn't hear anything",
					"like that. But I do sleep very soundly as I said and",
					"wouldn't necessarily have heard it if there was any such noise");
			}
		} else if (menu == 3) {
			if (n.getID() == NpcId.HOBBES_THE_BUTLER.id()) {
				npcsay(p, n, "Well, I do know that Elizabeth was extremely",
					...
					"I hate mosquitos.");
				say(p, n, "Yeah, so do I");
				npcsay(p, n, "you'd really have to ask her though.");
			} else if (n.getID() == NpcId.STANFORD_THE_GARDENER.id()) {
				npcsay(p, n, "Well, Bob mentioned to me the other day",
				...
				npcsay(p, n, "Well, I know David said that he was",
				...
				npcsay(p, n, "I told Carol to buy some from that strange",
				...
				npcsay(p, n, "I overheard Anna saying to Stanford",
				...
				npcsay(p, n, "Well, I do know Frank bought some poison",
	...
	public void threadDialogue(Player p, Npc n) {
		say(p, n, "I have proof that it wasn't any of the servants");
		p.message("you show the guard the thread you found on the window");
		say(p, n, "All the servants dress in black so",
			"it couldn't have been one of them");
		npcsay(p, n, "Thats some good work there. I guess it wasn't a servant.",
			"You still havent proved who did do it though");
	}

	public void evidenceDialogue(Player p, Npc n) {
		if (p.getCache().hasKey("p_anna2")) {
			say(p, n, "I have proof that Anna is lying about the poison");
			npcsay(p, n, "Oh really? How did you get that?");
			p.message("you tell the guard about the compost heap");
		} else if (p.getCache().hasKey("p_carol2")) {
			say(p, n, "I have proof that Carol is lying about the poison");
			npcsay(p, n, "Oh really? How did you get that?");
			p.message("you tell the guard about the drain");
		} else if (p.getCache().hasKey("p_eliza2")) {
			say(p, n, "I have proof that Elizabeth is lying about the poison");
			npcsay(p, n, "Oh really? How did you get that?");
			p.message("you tell the guard about the mosquitos at the fountain");
		} else if (p.getCache().hasKey("p_bob2")) {
			say(p, n, "I have proof that Bob is lying about the poison");
			npcsay(p, n, "Oh really? How did you get that?");
			p.message("you tell the guard about the beehive");
		} else if (p.getCache().hasKey("p_frank2")) {
			say(p, n, "I have proof that Frank is lying about the poison");
			npcsay(p, n, "Oh really? How did you get that?");
			p.message("you tell the guard about the tarnished family crest");
		} else if (p.getCache().hasKey("p_david2")) {
			say(p, n, "I have proof that David is lying about the poison");
			npcsay(p, n, "Oh really? How did you get that?");
			p.message("you tell the guard about the spiders nest");
		}
		npcsay(p, n, "Hmm. thats some good detective work there.",
			"We need more evidence before we can close the case though",
			"Keep up the good work");
	}

	public void fingerprintDialogue(Player p, Npc n) {
		say(p, n, "I have the fingerprints of the culprit");
		if (p.getCache().hasKey("murder_david")) {
			say(p, n, "I have Davids' Fingerprints here.");
		} else if (p.getCache().hasKey("murder_bob")) {
			say(p, n, "I have Bobs' Fingerprints here.");
		} else if (p.getCache().hasKey("murder_anna")) {
			say(p, n, "I have Annas' Fingerprints here.");
		} else if (p.getCache().hasKey("murder_eliz")) {
			say(p, n, "I have Elizabeths' Fingerprints here.");
		} else if (p.getCache().hasKey("murder_frank")) {
			say(p, n, "I have Franks' Fingerprints here.");
		} else if (p.getCache().hasKey("murder_carol")) {
			say(p, n, "I have Carols' Fingerprints here.");
		}
		say(p, n, "You can see for yourself they match the",
			"Fingerprints on the murder weapon exactly");
		p.message("You show the guard the finger prints evidence");
		npcsay(p, n, "...");
		npcsay(p, n, "I'm impressed. How on earth did you think",
		...
		if (i.getID() == ItemId.A_SILVER_DAGGER.id()) {
			switch (p.getQuestStage(this)) {
				...
				case 1:
					...
					if (!p.getCarriedItems().hasCatalogID(ItemId.A_SILVER_DAGGER.id(), Optional.of(false))) {
						give(p, ItemId.A_SILVER_DAGGER.id(), 1);
					} else {
			...
			switch (p.getQuestStage(this)) {
				...
				case 1:
					...
					if (!p.getCarriedItems().hasCatalogID(ItemId.MURDER_SCENE_POT.id(), Optional.of(false))) {
						give(p, ItemId.MURDER_SCENE_POT.id(), 1);
					} else {
			...
			switch (p.getQuestStage(this)) {
				...
				case 1:
					...
					if (!p.getCache().hasKey("thread") && !p.getCarriedItems().hasCatalogID(ItemId.THREAD_GREEN.id())
						...
						if (p.getCache().hasKey("murder_david")) {
							give(p, ItemId.THREAD_GREEN.id(), 1);
						} else if (p.getCache().hasKey("murder_anna")) {
							give(p, ItemId.THREAD_GREEN.id(), 1);
						} else if (p.getCache().hasKey("murder_carol")) {
							give(p, ItemId.THREAD_RED.id(), 1);
						} else if (p.getCache().hasKey("murder_frank")) {
							give(p, ItemId.THREAD_BLUE.id(), 1);
						} else if (p.getCache().hasKey("murder_eliz")) {
							give(p, ItemId.THREAD_BLUE.id(), 1);
						} else if (p.getCache().hasKey("murder_bob")) {
							give(p, ItemId.THREAD_RED.id(), 1);
				...
				case 1:
					if (obj.getID() == 1133) {
						...
						if (!p.getCarriedItems().hasCatalogID(ItemId.BOBS_SILVER_TEACUP.id(), Optional.of(false))) {
							...
							give(p, ItemId.BOBS_SILVER_TEACUP.id(), 1);
						} else {
						...
						if (!p.getCarriedItems().hasCatalogID(ItemId.ANNAS_SILVER_NECKLACE.id(), Optional.of(false))) {
							...
							give(p, ItemId.ANNAS_SILVER_NECKLACE.id(), 1);
						} else {
						...
						if (!p.getCarriedItems().hasCatalogID(ItemId.ELIZABETHS_SILVER_NEEDLE.id(), Optional.of(false))) {
							...
							give(p, ItemId.ELIZABETHS_SILVER_NEEDLE.id(), 1);
						} else {
						...
						if (!p.getCarriedItems().hasCatalogID(ItemId.FRANKS_SILVER_POT.id(), Optional.of(false))) {
							...
							give(p, ItemId.FRANKS_SILVER_POT.id(), 1);
						} else {
						...
						if (!p.getCarriedItems().hasCatalogID(ItemId.DAVIDS_SILVER_BOOK.id(), Optional.of(false))) {
							...
							give(p, ItemId.DAVIDS_SILVER_BOOK.id(), 1);
						} else {
						...
						if (!p.getCarriedItems().hasCatalogID(ItemId.CAROLS_SILVER_BOTTLE.id(), Optional.of(false))) {
							...
							give(p, ItemId.CAROLS_SILVER_BOTTLE.id(), 1);
						} else {
