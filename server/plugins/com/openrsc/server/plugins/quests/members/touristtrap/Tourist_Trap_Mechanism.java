...
public class Tourist_Trap_Mechanism implements RemoveObjTrigger, UseNpcTrigger, OpLocTrigger, UseLocTrigger, UseInvTrigger, TakeObjTrigger,
		...
		if (item.getCatalogId() == ItemId.TECHNICAL_PLANS.id() && npc.getID() == NpcId.BEDABIN_NOMAD_GUARD.id()) {
			...
			if (!player.getCarriedItems().hasCatalogID(ItemId.ANA_IN_A_BARREL.id(), Optional.of(false))) {
				...
				if (player.getCache().hasKey("ana_lift") || player.getCache().hasKey("ana_cart")
					...
					mes(player, "Oh, here's Ana, the guards must have discovered her.",
	...
	private void makeDartTip(Player player, GameObject obj) {
		if (obj.getID() == 1006) {
			if (!player.getCarriedItems().hasCatalogID(ItemId.TECHNICAL_PLANS.id(), Optional.of(false))) {
				mes(player, "This anvil is experimental...",
					"You need detailed plans of the item you want to make in order to use it.");
				return;
			}
			mes(player, "Do you want to follow the technical plans ?");
			...
			if (menu == 0) {
				...
				mes(player, "You begin experimenting in forging the weapon...",
				...
				if (succeedRate(player)) {
					mes(player, "You finally manage to forge a sharp, pointed...",
						"... dart tip...");
					...
					mes(player, "You study the technical plans even more...",
						"You need to attach feathers to the tip to complete the weapon.");
				} else {
					mes(player, "You waste the bronze bar through an unlucky accident.");
				}
			} else if (menu == 1) {
	...
	private void attachFeathersToPrototype(Player player, Item i, Item i2) {
		if (compareItemsIds(i, i2, ItemId.FEATHER.id(), ItemId.PROTOTYPE_DART_TIP.id())) {
			...
			mes(player, "You try to attach feathers to the bronze dart tip.",
			...
			if (succeedRate(player)) {
				...
				mes(player, "An unlucky accident causes you to waste the feathers.",
		...
		if (obj.getID() == DISTURBED_SAND1) {
			if (command.equals("look")) {
				if (player.getQuestStage(Quests.TOURIST_TRAP) <= 0) {
					mes(player, "You see some footsteps in the sand.");
				} else {
					mes(player, "This looks like some disturbed sand.",
						"footsteps seem to be heading of towards the south west.");
				}
			} else if (command.equals("search")) {
				if (player.getQuestStage(Quests.TOURIST_TRAP) <= 0) {
					mes(player, "You just see some footsteps in the sand.");
				} else {
					mes(player, "You search the footsteps more closely.",
		...
		//closest to camp
			...
			if (command.equals("look")) {
				if (player.getQuestStage(Quests.TOURIST_TRAP) <= 0) {
					...
					mes(player, "You find footsteps heading south.",
				...
				if (player.getQuestStage(Quests.TOURIST_TRAP) <= 0) {
					...
					mes(player, "You search the area thoroughly...",
			...
			mes(player, "You walk into the dark of the cavern...");
			...
			if (player.getCache().hasKey("ana_is_up")) {
				...
				mes(player, "You find the barrel with ana in it.",
			...
			if (player.getCache().hasKey("ana_lift")) {
				mes(player, "You see a barrel coming to the surface.",
			...
			player.message("You search the mine cart.");
			if (player.getCarriedItems().hasCatalogID(ItemId.ANA_IN_A_BARREL.id(), Optional.of(false))) {
				mes(player, "There should be enough space for Ana (in the barrel) to go on here.");
			}
			if (player.getCache().hasKey("ana_in_cart")) {
				mes(player, "You can see the barrel with Ana in it on the cart already.");
			}
			mes(player, "There is space on the cart for you get on, would you like to try?");
			...
			if (menu == 0) {
				...
				if (player.getCache().hasKey("ana_in_cart")) {
					mes(player, "You hear Ana starting to bang on the barrel for her to be let out.");
					mes(player, "@gre@Ana: Get me out of here, I'm suffocating!",
						"@gre@Ana: It smells like dwarven underwear in here!");
				}
				player.teleport(86, 808);
				if (player.getCache().hasKey("rescue")) {
					mes(player, "As soon as you get on the cart, it starts to move.",
	...
	private void getOutWithAnaInCart(Player player, Npc n, int cID) {
		if (cID == -1) {
			mes(player, "The cart driver seems to be festidiously cleaning his cart.",
		...
		switch (cID) {
			case CartDriver.PSSST:
				mes(player, "The cart driver completely ignores you.");
			...
			case CartDriver.PSSST2:
				mes(player, "The driver completely ignores you.");
			...
			case CartDriver.PSSSTFINAL:
				mes(player, "The cart driver turns around quickly to face you.");
			...
			case CartDriver.NICECART:
				mes(player, "The cart driver looks around at you and tries to weigh you up.");
				npcsay(player, n, "Hmmm.");
				mes(player, "He tuts to himself and starts checking the wheels.");
				npcsay(player, n, "Tut !");
			...
			case CartDriver.WAGON:
				mes(player, "The cart driver smirks a little.",
				...
				if (menu == 0) {
					...
					mes(player, "The cart driver smiles a bit and then turns to you.");
					...
					if (menu2 == 0) {
						...
						mes(player, "The cart driver checks that the guards aren't watching him.");
						...
						if (menu3 == 0) {
							...
							mes(player, "The cart driver laughs at your pun...");
							npcsay(player, n, "Ha ha ha, oh Stoppit!");
							mes(player, "The cart driver seems much happier now.");
							...
							if (menu4 == 0) {
								...
								mes(player, "The cart driver points at a nearby guard.");
								...
								if (menu5 == 0) {
									...
									mes(player, "The cart driver quickly starts preparing the cart.");
									int menu6 = multi(player, n,
										"Good luck!",
										"You can't leave me here, I'll get killed!");
									if (menu6 == 0) {
										npcsay(player, n, "Yeah, you too!");
										mes(player, "The cart sets off at a hectic pace.",
			...
			case CartDriver.HELPYOU:
				...
				if (help == 0) {
					...
					mes(player, "The cart driver seems mortally offended...",
			...
			case CartDriver.WONDERIF:
				...
				mes(player, "The cart driver chuckles to himself.");
			...
			case CartDriver.HECKOUT:
				...
				mes(player, "The cart driver goes back to his work.");
				break;
		}
	...
	private void driverCallGuards(Player player, Npc n) {
		...
		if (succeed == 0) {
			mes(player, "Some guards notice you and come over.");
			...
			mes(player, "You quickly slope away and hide from the guards.");
		}
	}
	...
	private void failCaveAnaInBarrel(Player player, Npc n) {
		if (player.getCarriedItems().hasCatalogID(ItemId.ANA_IN_A_BARREL.id(), Optional.of(false))) {
			...
			npcsay(player, n, "'Cor! This barrel is really heavy!",
				...
				"Har, har har!");
			mes(player, "@gre@Ana: How rude! Why I ought to teach you a lesson.");
			...
			mes(player, "@yel@Guards: Into the cell you go!",
	...
	private void anaToLift(Player player, Npc n) {
		...
		if (menu == 0) {
			...
			mes(player, "@gre@Ana in a barrel: Why you cheeky....!",
			...
			int opt = multi(player, n,
				...
				"Oh, nothing.");
			if (opt == 0) {
				mes(player, "@gre@Ana in a barrel: You creep!");
	...
	public void onUseLoc(Player player, GameObject obj, Item item) {
		if (obj.getID() == 1006 && item.getCatalogId() == ItemId.BRONZE_BAR.id()) {
			...
			mes(player, "You carefully place Ana in the barrel into the mine cart.",
			...
			mes(player, "You place Ana (In the barrel) carefully on the cart.",
		...
		if (item.getCatalogId() == ItemId.ANA_IN_A_BARREL.id()) {
			...
			if (menu == 0) {
				if (outsideCamp(player)) {
					mes(player, "@gre@Ana: You can't drop me here!",
				...
				mes(player, "You drop the barrel to the floor and Ana gets out.");
				player.getCarriedItems().remove(new Item(ItemId.ANA_IN_A_BARREL.id()));
				...
				mes(player, "Ana's outburst attracts the guards, they come running over.");
				...
				mes(player, "The guards drag Ana away and then throw you into a cell.");
				player.teleport(75, 3626);
			} else if (menu == 1) {
				mes(player, "You think twice about dropping the barrel to the floor.");
			}
		}
	}
