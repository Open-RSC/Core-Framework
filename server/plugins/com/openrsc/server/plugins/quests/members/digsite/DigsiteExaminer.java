		...
		if (n.getID() == NpcId.EXAMINER.id()) {
			if (cID == -1) {
				switch (p.getQuestStage(Quests.DIGSITE)) {
					case -1:
						...
						if (finalMenu == 1) {
							if (p.getCarriedItems().hasCatalogID(ItemId.TROWEL.id(), Optional.of(false))) {
								...
								give(p, ItemId.TROWEL.id(), 1);
								npcsay(p, n, "Deary me.. that was a good one as well",
					...
					case 0:
						...
						say(p, n, "earth sciences ?");
						npcsay(p, n, "That is right dear",
						...
						if (menu == 0) {
							 ...
							 */
							npcsay(p, n, "You can if you get this letter of recommendation stamped",
								"By the curator of varrock museum");
							say(p, n, "Oh right, I'll see what I can do");
							give(p, ItemId.UNSTAMPED_LETTER_OF_RECOMMENDATION.id(), 1);
							p.updateQuestStage(Quests.DIGSITE, 1);
						} else if (menu == 1) {
							npcsay(p, n, "You could gain much with an understanding of the world below");
						}
						break;
					case 1:
						say(p, n, "Hello");
						...
						if (p.getCarriedItems().hasCatalogID(ItemId.STAMPED_LETTER_OF_RECOMMENDATION.id(), Optional.of(false))) {
							...
							npcsay(p, n, "I am still waiting for your stamped letter of recommendation");
							int opt = multi(p, n,
							...
							if (opt == 0) {
								if (p.getCarriedItems().hasCatalogID(ItemId.UNSTAMPED_LETTER_OF_RECOMMENDATION.id(), Optional.of(false))) {
									npcsay(p, n, "Oh now come on",
										"You have it with you!");
								} else {
									npcsay(p, n, "That was foolish!",
										"Take this one and keep it safe this time...");
									give(p, ItemId.UNSTAMPED_LETTER_OF_RECOMMENDATION.id(), 1);
								}
							} else if (opt == 1) {
								npcsay(p, n, "I am sure you wont get any problems");
							}
						}
						break;
					case 2:
						...
						if (opt2 == 0) {
							say(p, n, "Yes I certainly am");
							digsiteExaminerDialogue(p, n, ExaminerNPC.START_EXAM_AND_MENU_ONE);
						} else if (opt2 == 1) {
							say(p, n, "Sorry, I didn't mean to disturb you...");
							npcsay(p, n, "Oh, no problem at all");
						}
						break;
					case 3:
						say(p, n, "Hello");
						...
						if (opt3 == 0) {
							...
							npcsay(p, n, "Well well, have you not been doing your studies ?",
			...
			switch (cID) {
				case ExaminerNPC.START_EXAM_AND_MENU_ONE:
					npcsay(p, n, "Okay, we will start with the first level exam:",
					...
					if (p.getCache().hasKey("student_orange_c")
						...
						menu1 = multi(p, n, false, //do not send over
					...
					if (menu1 == 0) {
						if (p.getCache().hasKey("student_orange_c")
							&& p.getCache().hasKey("student_green_c")
							&& p.getCache().hasKey("student_purple_c")) {
							say(p, n, "The study of the earth, It's contents and It's history");
							CORRECT_ANSWERS++;
						} else {
							say(p, n, "The study of gardening, planting and fruiting vegetation");
						}
						digsiteExaminerDialogue(p, n, ExaminerNPC.START_EXAM_MENU_TWO);
					} else if (menu1 == 1) {
						say(p, n, "The study of planets, and the history of forming worlds");
						digsiteExaminerDialogue(p, n, ExaminerNPC.START_EXAM_MENU_TWO);
					} else if (menu1 == 2) {
						say(p, n, "The combination of all skills applied to the working of the earth");
						digsiteExaminerDialogue(p, n, ExaminerNPC.START_EXAM_MENU_TWO);
					}
					break;
				case ExaminerNPC.START_EXAM_MENU_TWO:
					npcsay(p, n, "Okay, next question...",
				...
				case ExaminerNPC.START_EXAM_FINAL:
					...
					if (p.getCache().hasKey("student_orange_c")
						...
						npcsay(p, n, "You got all the questions correct, well done");
						say(p, n, "Hey! Excellent!");
						npcsay(p, n, "You have now passed the Earth sciences level 1 general exam",
						...
						give(p, ItemId.TROWEL.id(), 1);
						give(p, ItemId.LEVEL_1_CERTIFICATE.id(), 1);
						...
						if (CORRECT_ANSWERS == 0) {
							...
							npcsay(p, n, "You got 1 question correct",
								"Better luck next time");
							say(p, n, "Oh bother!");
						} else if (CORRECT_ANSWERS == 2) {
							npcsay(p, n, "You got 2 questions correct",
				...
				case ExaminerNPC.START_EXAM2_AND_MENU_ONE:
					npcsay(p, n, "Okay, this is the next part of the earth sciences exam",
				...
				case ExaminerNPC.START_EXAM2_FINAL:
					...
					if (p.getCache().hasKey("student_orange_exam2")
						...
						npcsay(p, n, "You got all the questions correct, well done!");
						say(p, n, "Great, I'm getting good at this");
						npcsay(p, n, "You have now passed the Earth sciences level 2 intermediate exam",
							"Here is your certificate");
						give(p, ItemId.LEVEL_2_CERTIFICATE.id(), 1);
						...
						if (CORRECT_ANSWERS == 0) {
							...
							say(p, n, "Nearly got it");
						}
					}
					break;
				case ExaminerNPC.START_EXAM3_AND_MENU_ONE:
					npcsay(p, n, "Attention, this is the final part of the earth sciences exam",
				...
				case ExaminerNPC.START_EXAM3_FINAL:
					...
					if (p.getCache().hasKey("student_orange_exam3")
						...
						npcsay(p, n, "You got all the questions correct, well done!");
						say(p, n, "Hooray!");
						npcsay(p, n, "Congratulations, You have now passed the Earth sciences level 3 advanced exam",
							"Here is your level 3 certificate");
						give(p, ItemId.LEVEL_3_CERTIFICATE.id(), 1);
						say(p, n, "I can dig wherever I want now...");
						...
						if (CORRECT_ANSWERS == 0) {
							npcsay(p, n, "I cannot believe this!",
