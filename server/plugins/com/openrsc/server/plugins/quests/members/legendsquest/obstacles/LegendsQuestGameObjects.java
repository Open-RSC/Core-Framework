	...
	public void onOpLoc(Player player, GameObject obj, String command) {
		if (obj.getID() == ECHNED_ZEKIN_ROCK) {
			if (player.getQuestStage(Quests.LEGENDS_QUEST) == 8) {
				mes(player, config().GAME_TICK * 2, "The rock moves quite easily.");
			...
			if (echned == null) {
				mes(player, config().GAME_TICK * 2, "A thick, green mist seems to emanate from the water...",
				...
				if (echned != null) {
					...
					mes(player, echned, config().GAME_TICK * 2, "Which slowly floats towards you.");
			...
			if (command.equalsIgnoreCase("enter")) {
				if (player.getY() >= 3733) {
					...
					if (player.getCache().hasKey("cavernous_opening") || player.getQuestStage(Quests.LEGENDS_QUEST) == -1) {
						mes(player, config().GAME_TICK * 2, "You walk carefully into the darkness of the cavern..");
						player.teleport(395, 3733);
					} else {
						mes(player, config().GAME_TICK * 2, "You walk into an invisible barrier...");
						mes(player, config().GAME_TICK, "Somekind of magical force will not allow you to pass into the cavern.");
					}
				}
			} else if (command.equalsIgnoreCase("search")) {
				if (player.getCache().hasKey("cavernous_opening")) {
					mes(player, config().GAME_TICK * 2, "You can see a glowing crystal shape in the wall.",
						"It looks like the Crystal is magical, ",
						"it allows access to the cavern.");
				} else {
					mes(player, config().GAME_TICK * 2, "You see a heart shaped depression in the wall next to the cavern.",
						"And a message reads...",
						"@gre@All ye who stand 'ere the dragons teeth,");
					mes(player, config().GAME_TICK, "@gre@Place your full true heart and proceed...");
				}
			}
		}
		else if (obj.getID() == ANCIENT_LAVA_FURNACE) {
			if (command.equalsIgnoreCase("look")) {
				mes(player, config().GAME_TICK, "This is an ancient looking furnace.");
			} else if (command.equalsIgnoreCase("search")) {
				mes(player, config().GAME_TICK * 2, "You search the lava furnace.",
					"You find a small compartment that you may be able to use.",
					"Strangely, it looks as if it is designed for a specific purpose...");
				mes(player, config().GAME_TICK, "to fuse things together at very high temperatures...");
			}
		}
		else if (obj.getID() == RED_EYE_ROCK) {
			mes(player, config().GAME_TICK, "These rocks look somehow manufactured..");
		}
			...
			if (player.getQuestStage(Quests.LEGENDS_QUEST) >= 9 || blockDescendBeamPostQuest(player)) {
				mes(player, config().GAME_TICK * 2, "The rope snaps as you're about to climb down it.",
					"Perhaps you need a new rope.");
				return;
			}
			mes(player, config().GAME_TICK * 2, "This rope climb looks pretty dangerous,",
				"Are you sure you want to go down?");
			int menu = multi(player,
				"Yes,I'll go down the rope...",
				"No way do I want to go down there.");
			if (menu == 0) {
				mes(player, config().GAME_TICK * 2, "You prepare to climb down the rope...");
				...
				if ((player.getQuestStage(Quests.LEGENDS_QUEST) >= 0 && !player.getCache().hasKey("gujuo_potion")) || blockDescendBeamPostQuest(player)) {
					mes(player, config().GAME_TICK * 2, "...but a terrible fear grips you...");
					player.message("And you can go no further.");
				} else {
					int rnd = DataConversions.random(0, 4);
					if (rnd == 0) {
						mes(player, config().GAME_TICK * 2, "but fear stabs at your heart...",
								"and you lose concentration,",
							"you slip and fall....");
						player.damage(DataConversions.random(10, 15));
					}
					else {
						mes(player, config().GAME_TICK * 2, "And although fear stabs at your heart...",
			...
			if (player.getCache().hasKey("legends_wooden_beam")) {
				...
				mes(player, config().GAME_TICK * 2, "You see nothing special about this...");
				player.message("Perhaps if you had a rope, it might be more functional.");
			}
		}
		else if (obj.getID() == CARVED_ROCK) {
			mes(player, config().GAME_TICK * 2, "You see a delicate inscription on the rock, it says,");
			mes(player, config().GAME_TICK * 3, "@gre@'Once there were crystals to make the pool shine,'");
			...
			if (!gem.equals("") && attached) {
				mes(player, config().GAME_TICK * 2, "A barely visible " + gem + " becomes clear again, spinning above the rock.",
			...
			if (command.equalsIgnoreCase("look")) {
				mes(player, config().GAME_TICK * 2, "This huge metal gate bars the way further...",
				...
				mes(player, config().GAME_TICK * 2, "You push the gates...they're very stiff...",
				...
				if (menu == 0) {
					...
					mes(player, config().GAME_TICK * 2, "You ripple your muscles...preparing too exert yourself...");
					say(player, null, "Hup!");
					mes(player, config().GAME_TICK * 2, "You brace yourself against the doors...");
					say(player, null, "Urghhhhh!");
					mes(player, config().GAME_TICK * 2, "You start to force against the gate..");
					say(player, null, "Arghhhhhhh!");
					mes(player, config().GAME_TICK * 2, "You push and push,");
					say(player, null, "Shhhhhhhshshehshsh");
					if (Formulae.failCalculation(player, Skills.STRENGTH, 50)) {
						mes(player, config().GAME_TICK * 2, "You just manage to force the gates open slightly, ",
						...
						mes(player, config().GAME_TICK * 2, "but run out of steam before you're able to force the gates open.");
			...
			if (player.getCarriedItems().hasCatalogID(Mining.getAxe(player), Optional.of(false))) {
				...
				if (Formulae.failCalculation(player, Skills.MINING, 50)) {
					mes(player, config().GAME_TICK * 2, "You take a good swing at the rock with your pick...");
					...
					mes(player, config().GAME_TICK * 3, "...and smash it into smaller pieces.");
			...
			if (command.equalsIgnoreCase("open")) {
				if (player.getY() >= 3703) {
					mes(player, config().GAME_TICK * 2, "You push the doors open and walk through.");
					...
					mes(player, config().GAME_TICK * 2, "You push on the doors...they're really shut..",
				...
				if (player.getY() >= 3703) {
					mes(player, config().GAME_TICK * 2, "You see a lever which you pull on to open the door.");
					changeloc(obj, config().GAME_TICK * 3, 497);
					player.teleport(442, 3701);
					mes(player, config().GAME_TICK * 2, "You walk through the door.");
					player.message("The doors make a satisfying 'CLICK' sound as they close.");
				} else {
					...
					if (player.getCarriedItems().hasCatalogID(ItemId.LOCKPICK.id(), Optional.of(false))) {
						mes(player, config().GAME_TICK * 2, "You attempt to pick the lock..");
						...
						if (Thieving.succeedPickLockThieving(player, 50)) {
							mes(player, config().GAME_TICK * 2, "@gre@'CLICK'");
							say(player, null, "Easy as pie...");
							delay(config().GAME_TICK * 2);
							mes(player, config().GAME_TICK * 2, "You tumble the lock mechanism and the door opens easily.");
						...
						mes(player, config().GAME_TICK * 2, "The mechanism for this lock looks very sophisticated...");
			...
			if (player.getCarriedItems().hasCatalogID(ItemId.SHAMANS_TOME.id(), Optional.empty())) {
				mes(player, config().GAME_TICK * 2, "You search the desk ...");
				player.message("...but find nothing.");
			} else {
				mes(player, config().GAME_TICK * 4, "You search the desk ...");
			...
			mes(player, config().GAME_TICK * 2, "You search the bookcase...",
			...
			int menu = multi(player,
				...
				"No, I'll stay here.");
			if (menu == 0) {
				mes(player, config().GAME_TICK * 2, "You climb through the hole in the wall..",
			...
			if (player.getCarriedItems().hasCatalogID(ItemId.SCRAWLED_NOTES.id(), Optional.empty())) {
				...
				mes(player, config().GAME_TICK * 2, "You find a scrap of paper with nonesense written on it.");
			}
		}
			...
			if (player.getCarriedItems().hasCatalogID(ItemId.SCATCHED_NOTES.id(), Optional.empty())) {
				...
				mes(player, config().GAME_TICK * 2, "You find a scrap of paper with spidery writing on it.");
			}
		}
			...
			if (player.getCarriedItems().hasCatalogID(ItemId.SCRIBBLED_NOTES.id(), Optional.empty())) {
				...
				mes(player, config().GAME_TICK * 2, "After some time you find a scrumpled up piece of paper.");
				player.message("It looks like rubbish...");
			}
		}
		else if (obj.getID() == CAVE_ENTRANCE_FROM_BOULDERS) {
			mes(player, config().GAME_TICK * 2, "You see a small cave entrance.",
			...
			mes(player, config().GAME_TICK * 2, "You crawl back out from the cavern...");
			...
			mes(player, config().GAME_TICK * 2, "These tall reeds look nice and long, ");
			mes(player, config().GAME_TICK * 2, "with a long tube for a stem.");
			...
			if (player.getCache().hasKey("legends_cavern") || player.getQuestStage(Quests.LEGENDS_QUEST) >= 2 || player.getQuestStage(Quests.LEGENDS_QUEST) == -1) {
				if (player.getQuestStage(Quests.LEGENDS_QUEST) == 1) {
					mes(player, config().GAME_TICK * 2, "You see nothing significant...",
						"At first....");
				}
				mes(player, config().GAME_TICK * 2, "You see that there is a small crevice that you may be able to crawl though.?",
				...
				if (menu == 0) {
					...
					mes(player, config().GAME_TICK * 2, "You try to crawl through...",
						"You contort your body to fit the crevice.");
					if (Formulae.failCalculation(player, Skills.AGILITY, 50)) {
						mes(player, config().GAME_TICK * 2, "You adroitely squeeze serpent like into the crevice.",
						...
						mes(player, config().GAME_TICK * 5, "You get cramped into a tiny space and start to suffocate.",
							"You wriggle and wriggle but you cannot get out..");
						mes(player, config().GAME_TICK * 2, "Eventually you manage to break free.",
							"But you scrape yourself very badly as your force your way out.",
							"And you're totally exhausted from the experience.");
						player.damage(5);
					}
				} else if (menu == 1) {
					mes(player, config().GAME_TICK * 2, "You decide against forcing yourself into the tiny crevice..",
			...
			if (player.getQuestStage(Quests.LEGENDS_QUEST) >= 10 || player.getQuestStage(Quests.LEGENDS_QUEST) == -1) {
				...
				mes(player, config().GAME_TICK * 2, "This totem pole is truly awe inspiring.",
			...
			mes(player, config().GAME_TICK * 2, "This totem pole looks very corrupted,",
			...
			if (player.getQuestStage(Quests.LEGENDS_QUEST) >= 1 || player.getQuestStage(Quests.LEGENDS_QUEST) == -1) {
				if (player.getCarriedItems().hasCatalogID(ItemId.MACHETTE.id(), Optional.of(false))) {
					...
					mes(player, config().GAME_TICK * 2, "You open the cupboard and find a machette.",
			...
			if (obj.getOwner().equals(player.getUsername())) {
				mes(player, config().GAME_TICK * 2, "This totem pole looks very heavy...");
	...
	public void onUseLoc(Player player, GameObject obj, Item item) {
		if (obj.getID() == TOTEM_POLE && item.getCatalogId() == ItemId.TOTEM_POLE.id()) {
			...
			if (obj.getOwner().equals(player.getUsername())) {
				...
				mes(player, config().GAME_TICK * 2, "You craft a totem pole out of the Yommi tree.");
			...
			if (obj.getOwner().equals(player.getUsername())) {
				int objectX = obj.getX();
				int objectY = obj.getY();
				mes(player, config().GAME_TICK * 2, "You professionally wield your Rune Axe...",
			...
			if (obj.getOwner().equals(player.getUsername())) {
				int objectX = obj.getX();
				int objectY = obj.getY();
				mes(player, config().GAME_TICK * 2, "You wield the Rune Axe and prepare to chop the Yommi tree.");
				...
				mes(player, config().GAME_TICK * 2, "You chop the Yommi tree down.",
			...
			mes(player, config().GAME_TICK * 2, "You gain some logs..");
			changeloc(obj, new GameObject(obj.getWorld(), obj.getLocation(), FERTILE_EARTH, obj.getDirection(), obj.getType()));
			...
			mes(player, config().GAME_TICK * 2, "You water the Yommi tree from the golden bowl...",
			...
			mes(player, config().GAME_TICK * 2, "Soon the tree stops growing...",
			...
			if (DataConversions.random(0, 1) != 1) {
				...
				mes(player, config().GAME_TICK * 2, "You bury the Germinated Yommi tree seed in the fertile earth...",
			...
			mes(player, config().GAME_TICK * 2, "You carefully place the glowing heart shaped crystal into ",
			...
			mes(player, config().GAME_TICK * 2, "You carefully place the Dragon Crystal on the rock.",
			...
			switch (ItemId.getById(item.getCatalogId())) {
				...
				case A_HUNK_OF_CRYSTAL:
					...
					if (!player.getCache().hasKey(item.getDef(player.getWorld()).getName().toLowerCase().replace(" ", "_"))) {
						...
						player.getCarriedItems().remove(new Item(item.getCatalogId()));
						mes(player, config().GAME_TICK * 2, "You carefully place the piece of crystal into ",
					...
					if (player.getCache().hasKey("a_chunk_of_crystal") && player.getCache().hasKey("a_lump_of_crystal") && player.getCache().hasKey("a_hunk_of_crystal")) {
						mes(player, config().GAME_TICK * 2, "You place the final segment of the crystal together into the ",
						...
						mes(player, config().GAME_TICK * 2, "The compartment in the furnace isn't full yet.");
						mes(player, config().GAME_TICK, "It looks like you need more pieces of crystal.");
					}
					break;
				default:
			...
			switch (ItemId.getById(item.getCatalogId())) {
				...
				case RED_TOPAZ:
					...
					if (alreadyAttached) {
						...
						mes(player, config().GAME_TICK * 2, "A barely visible " + item.getDef(player.getWorld()).getName() + " becomes clear again, spinning above the rock.");
						...
						if (attachmentMode != -1 && !player.getCarriedItems().hasCatalogID(ItemId.BOOKING_OF_BINDING.id(), Optional.empty())) {
							...
							if (player.getCache().hasKey("legends_attach_1")
								...
								mes(player, config().GAME_TICK * 2, "Suddenly all the crystals begin to glow very brightly.",
			...
			mes(player, config().GAME_TICK * 2, "You use your machette to cut down a tall reed.",
			...
			mes(player, config().GAME_TICK * 2, "The water is awkward to get to...",
			...
			if (atQuestStages(player, Quests.LEGENDS_QUEST, 5, 6, 7)) {
				mes(player, config().GAME_TICK * 2, "It looks as if this pool has dried up...",
			...
			if((player.getQuestStage(Quests.LEGENDS_QUEST) >= 9 || player.getQuestStage(Quests.LEGENDS_QUEST) == -1)
				...
				mes(player, config().GAME_TICK * 2, "You use the cut reed plant to syphon some water from the pool.",
			...
			if (emptyID != ItemId.NOTHING.id()) {
				...
				if (emptyID == ItemId.GOLDEN_BOWL.id()) {
					mes(player, config().GAME_TICK * 2, "into your gold bowl.");
					...
					mes(player, config().GAME_TICK * 2, "The water doesn't seem to sparkle as much as it did in the pool.");
				} else if (emptyID == ItemId.BLESSED_GOLDEN_BOWL.id()) {
					mes(player, config().GAME_TICK * 2, "into your blessed gold bowl.");
					player.getCarriedItems().remove(new Item(ItemId.BLESSED_GOLDEN_BOWL.id()));
					player.getCarriedItems().getInventory().add(new Item(ItemId.BLESSED_GOLDEN_BOWL_WITH_PURE_WATER.id()));
					mes(player, config().GAME_TICK * 2, "The water seems to bubble and sparkle as if alive.");
				} else {
					mes(player, config().GAME_TICK * 2, "You put some water in the " + player.getWorld().getServer().getEntityHandler().getItemDef(emptyID).getName().toLowerCase() + ".");
				...
				mes(player, config().GAME_TICK * 2, "You start to syphon some water up the tube...");
