		...
		if (obj.getID() == LEGENDS_HALL_DOOR) {
			if (command.equalsIgnoreCase("open")) {
				if (player.getQuestStage(Quests.LEGENDS_QUEST) >= 11 || player.getQuestStage(Quests.LEGENDS_QUEST) == -1) {
					...
					mes(player.getWorld().getServer().getConfig().GAME_TICK * 2, "You need to complete the Legends Guild Quest");
					mes(player.getWorld().getServer().getConfig().GAME_TICK * 2, "before you can enter the Legends Guild");
				}
