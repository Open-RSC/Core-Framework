	...
	public static void demonFight(Player player) {
		...
		if (third_nezikchened != null) {
			...
			if (player.getCache().hasKey("viyeldi_companions") && player.getCache().getInt("viyeldi_companions") <= 3) {
				...
				mes(player, third_nezikchened, config().GAME_TICK * 2, "The Demon starts chanting...",
				...
				if (third_nezikchened != null) {
					...
					mes(player, config().GAME_TICK * 2, "The demon is summoning the dead hero's from the Viyeldi caves !");
				...
				mes(player, third_nezikchened, config().GAME_TICK * 2, "The Demon screams in rage...");
	...
	public void onKillNpc(Player player, Npc n) {
		if (n.getID() == NpcId.NEZIKCHENED.id()) {
			...
			if (player.getQuestStage(Quests.LEGENDS_QUEST) == 3 && player.getLocation().isInsideFlameWall()) {
				...
				mes(player, config().GAME_TICK, "Your opponent is retreating");
				if (n != null) {
					n.remove();
				}
				mes(player, config().GAME_TICK * 2, "The demon starts an incantation...",
				...
				player.damage(7);
				mes(player, config().GAME_TICK * 2, "@yel@Nezikchened : Haha hah ha ha ha ha....",
			...
			// SECOND FIGHT.
				...
				mes(player, config().GAME_TICK * 2, "The Demon seems very angry now...",
			...
			// THIRD FIGHT.
				...
				mes(player, config().GAME_TICK * 2, "You deliver the final killing blow to the foul demon.",
	...
	public void onAttackNpc(Player player, Npc affectedmob) {
		if (affectedmob.getID() == NpcId.NEZIKCHENED.id()) {
			if ((affectedmob.getAttribute("spawnedFor", null) != null && !affectedmob.getAttribute("spawnedFor").equals(player)) || !atQuestStages(player, Quests.LEGENDS_QUEST, 3, 7, 8)) {
				mes(player, config().GAME_TICK * 2, "Your attack glides straight through the Demon.");
				mes(player, config().GAME_TICK, "as if it wasn't really there.");
				if (affectedmob != null)
					affectedmob.remove();
			}
		}
