...
public final class Harvesting implements OpLocTrigger {
	...
	private void handleClipHarvesting(final GameObject object, final Player player,
		...
		if (!objName.contains("herb") && player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HARVESTING)) < reqLevel) {
		...
		int repeat = Formulae.getRepeatTimes(player, getSkillId(player.getWorld(), SkillsEnum.HARVESTING));
		startbatch(repeat);
	...
	private void batchClipping(Player player, GameObject object, String objName, HerbsProduce prodEnum) {
		...
		if (getProduce(prodEnum.get(prodId).getLevel(), player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HARVESTING)))) {
			...
			if (obj == null) {
				...
				player.incExp(getSkillId(player.getWorld(), SkillsEnum.HARVESTING), prodEnum.get(prodId).getXp(), true);
			}
	...
	private void batchHarvest(Player player, int toolId, GameObject object, ObjectHarvestingDef def) {
		...
		if (player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HARVESTING)) < def.getReqLevel()) {
		...
		} else if (getProduce(def.getReqLevel(), player.getSkills().getLevel(getSkillId(player.getWorld(), SkillsEnum.HARVESTING)))) {
			//check if the object is still up
			GameObject obj = player.getViewArea().getGameObject(object.getID(), object.getX(), object.getY());
			if (obj == null) {
				...
				player.incExp(getSkillId(player.getWorld(), SkillsEnum.HARVESTING), def.getExp(), true);
			}
