...
public class DataFileDecrypter {
    ...
    private static void method131(DataFileVariables dataFileVariables) {
        ...
        while (flag19) {
            ...
            byte0 = method132(dataFileVariables);
            dataFileVariables.anInt482 = dataFileVariables.anInt482 << 8 | byte0 & 0xff;
            byte0 = method132(dataFileVariables);
            dataFileVariables.anInt482 = dataFileVariables.anInt482 << 8 | byte0 & 0xff;
            byte0 = method132(dataFileVariables);
            dataFileVariables.anInt482 = dataFileVariables.anInt482 << 8 | byte0 & 0xff;
            ...
            for (int l3 = 0; l3 < j4; l3++) {
                ...
                method136(dataFileVariables.anIntArrayArray499[l3], dataFileVariables.anIntArrayArray500[l3], dataFileVariables.anIntArrayArray501[l3], dataFileVariables.aByteArrayArray498[l3], byte8, i, i4);
            ...
            int l7;
            byte byte9;
            for (l7 = method134(i7, dataFileVariables); l7 > ai[i7]; l7 = l7 << 1 | byte9) {
                i7++;
            ...
            for (int k5 = ai2[l7 - ai1[i7]]; k5 != l4;)
                if (k5 == 0 || k5 == 1) {
                    ...
                    do {
                        ...
                        int i8;
                        byte byte10;
                        for (i8 = method134(j7, dataFileVariables); i8 > ai[j7]; i8 = i8 << 1 | byte10) {
                            j7++;
                        ...
                        if (dataFileVariables.anIntArray495[0] == 0) {
                            ...
                            for (int k9 = 15; k9 >= 0; k9--) {
                                for (int l9 = 15; l9 >= 0; l9--) {
                                    dataFileVariables.aByteArray494[i10] = dataFileVariables.aByteArray494[dataFileVariables.anIntArray495[k9] + l9];
                                    i10--;
                                }
                    ...
                    int j8;
                    byte byte11;
                    for (j8 = method134(k7, dataFileVariables); j8 > ai[k7]; j8 = j8 << 1 | byte11) {
                        k7++;
            ...
            if (dataFileVariables.anInt486 == dataFileVariables.anInt503 + 1 && dataFileVariables.anInt476 == 0)
                flag19 = true;
            else
                flag19 = false;
        }
    ...
    private static int method134(int i, DataFileVariables dataFileVariables) {
        ...
        do {
            if (dataFileVariables.anInt479 >= i) {
                int k = dataFileVariables.anInt478 >> dataFileVariables.anInt479 - i & (1 << i) - 1;
    ...
    private static void method136(int ai[], int ai1[], int ai2[], byte abyte0[], int i, int j, int k) {
        int l = 0;
    ...
    public static byte[] load(InputStream input) throws IOException {
		...
		while (name_ < 2) {
			try {
				byte byte_arr[] = new byte[6];
				input.read(byte_arr);
				decmp_len = ((byte_arr[0] & 0xff) << 16) + ((byte_arr[1] & 0xff) << 8) + (byte_arr[2] & 0xff);
				ptr = ((byte_arr[3] & 0xff) << 16) + ((byte_arr[4] & 0xff) << 8) + (byte_arr[5] & 0xff);
				byte_arr2 = new byte[ptr];
				int read = 0;
				while(read != byte_arr2.length)
				{
					read += input.read(byte_arr2, read, byte_arr2.length - read);
				}
		...
		if (ptr != decmp_len) {
			byte byte_arr3[] = new byte[decmp_len];
			unpackData(byte_arr3, decmp_len, byte_arr2, ptr, 0);
			return byte_arr3;
		} else
			return byte_arr2;
	}
    ...
    public static byte[] loadCachedData(String entry, byte arc[]) {
		int cnt = arc[0] * 256 + arc[1];
		int name = 0;
		entry = entry.toUpperCase();
		for (int cur = 0; cur < entry.length(); cur++)
			name = (name * 61 + entry.charAt(cur)) - 32;
		int l = 2 + cnt * 10;
		for (int cur = 0; cur < cnt; cur++) {
			int name_ = (arc[cur * 10 + 2] & 0xff) * 0x1000000 + (arc[cur * 10 + 3] & 0xff) * 0x10000 + (arc[cur * 10 + 4] & 0xff) * 256 + (arc[cur * 10 + 5] & 0xff);
			int decmp_len = (arc[cur * 10 + 6] & 0xff) * 0x10000 + (arc[cur * 10 + 7] & 0xff) * 256 + (arc[cur * 10 + 8] & 0xff);
			int ptr = (arc[cur * 10 + 9] & 0xff) * 0x10000 + (arc[cur * 10 + 10] & 0xff) * 256 + (arc[cur * 10 + 11] & 0xff);
			if (name_ == name) {
				byte byte_arr2[] = new byte[decmp_len + 0];
				if (decmp_len != ptr)
					unpackData(byte_arr2, decmp_len, arc, ptr, l);
				else
					for (int i2 = 0; i2 < decmp_len; i2++)
						byte_arr2[i2] = arc[l + i2];
				return byte_arr2;
			}
			l += ptr;
		}
