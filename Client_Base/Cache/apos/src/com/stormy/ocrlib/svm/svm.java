    ...
    static double k_function(svm_node[] x, svm_node[] y, svm_parameter param) {
        switch (param.kernel_type) {
            ...
            case svm_parameter.RBF: {
                ...
                while (i < xlen && j < ylen) {
                    if (x[i].index == y[j].index) {
                        final double d = x[i++].value - y[j++].value;
                        sum += d * d;
...
class Solver {
    ...
    void reconstruct_gradient() {
        ...
        if (nr_free * l > 2 * active_size * (l - active_size)) {
            for (i = active_size; i < l; i++) {
                final float[] Q_i = Q.get_Q(i, active_size);
                for (j = 0; j < active_size; j++) {
                    if (is_free(j)) {
                        G[i] += alpha[j] * Q_i[j];
                    }
    ...
    void Solve(int l, QMatrix Q, double[] p_, byte[] y_, double[] alpha_,
        ...
        int iter = 0;
        final int max_iter = Math.max(10000000, l > Integer.MAX_VALUE / 100 ? Integer.MAX_VALUE : 100 * l);
        ...
        while (iter < max_iter) {
            ...
            if (y[i] != y[j]) {
                ...
                final double delta = (-G[i] - G[j]) / quad_coef;
                final double diff = alpha[i] - alpha[j];
                alpha[i] += delta;
                alpha[j] += delta;
    ...
    int select_working_set(int[] working_set) {
        ...
        for (int j = 0; j < active_size; j++) {
            if (y[j] == +1) {
                if (!is_lower_bound(j)) {
                    final double grad_diff = Gmax + G[j];
                    if (G[j] >= Gmax2) {
                        Gmax2 = G[j];
                    }
                    if (grad_diff > 0) {
                        double obj_diff;
                        final double quad_coef = QD[i] + QD[j] - 2.0 * y[i] * Q_i[j];
                        if (quad_coef > 0) {
...
final class Solver_NU extends Solver {
    ...
    int select_working_set(int[] working_set) {
        ...
        final int ip = Gmaxp_idx;
        final int in = Gmaxn_idx;
        float[] Q_ip = null;
        float[] Q_in = null;
        if (ip != -1) {
        ...
        for (int j = 0; j < active_size; j++) {
            if (y[j] == +1) {
                if (!is_lower_bound(j)) {
                    final double grad_diff = Gmaxp + G[j];
                    if (G[j] >= Gmaxp2) {
                        Gmaxp2 = G[j];
                    }
                    if (grad_diff > 0) {
                        double obj_diff;
                        final double quad_coef = QD[ip] + QD[j] - 2 * Q_ip[j];
                        if (quad_coef > 0) {
                            obj_diff = -(grad_diff * grad_diff) / quad_coef;
                        } else {
                            obj_diff = -(grad_diff * grad_diff) / 1e-12;
                ...
                if (!is_upper_bound(j)) {
                    final double grad_diff = Gmaxn - G[j];
                    if (-G[j] >= Gmaxn2) {
                        Gmaxn2 = -G[j];
                    }
                    if (grad_diff > 0) {
                        double obj_diff;
                        final double quad_coef = QD[in] + QD[j] - 2 * Q_in[j];
                        if (quad_coef > 0) {
                            obj_diff = -(grad_diff * grad_diff) / quad_coef;
                        } else {
                            obj_diff = -(grad_diff * grad_diff) / 1e-12;
...
public class svm {
    ...
    private static void solve_one_class(svm_problem prob, svm_parameter param,
        ...
        final int n = (int) (param.nu * prob.l); // # of alpha's at upper bound
    ...
    private static void svm_binary_svc_probability(svm_problem prob,
        ...
        final double[] dec_values = new double[prob.l];
        ...
        for (i = 0; i < nr_fold; i++) {
            ...
            final int end = (i + 1) * prob.l / nr_fold;
            int j, k;
            ...
            if (p_count == 0 && n_count == 0) {
                ...
                final svm_parameter subparam = (svm_parameter) param.clone();
                ...
                final svm_model submodel = svm_train(subprob, subparam);
                for (j = begin; j < end; j++) {
    ...
    public static svm_model svm_train(svm_problem prob, svm_parameter param) {
        final svm_model model = new svm_model();
        model.param = param;
        ...
        if (param.svm_type == svm_parameter.ONE_CLASS || param.svm_type == svm_parameter.EPSILON_SVR || param.svm_type == svm_parameter.NU_SVR) {
            ...
            final decision_function f = svm_train_one(prob, param, 0, 0);
            ...
            // classification
            final int l = prob.l;
            final int[] tmp_nr_class = new int[1];
            final int[][] tmp_label = new int[1][];
            final int[][] tmp_start = new int[1][];
            final int[][] tmp_count = new int[1][];
            final int[] perm = new int[l];

            // group training data of the same class
            svm_group_classes(prob, tmp_nr_class, tmp_label, tmp_start, tmp_count, perm);
            final int nr_class = tmp_nr_class[0];
            final int[] label = tmp_label[0];
            final int[] start = tmp_start[0];
            final int[] count = tmp_count[0];

            if (nr_class == 1) {
                svm.info("WARNING: training data in only one class. See README for details.\n");
            }

            final svm_node[][] x = new svm_node[l][];
            int i;
            for (i = 0; i < l; i++) {
                x[i] = prob.x[perm[i]];
            }
            ...
            for (i = 0; i < nr_class; i++) {
                for (int j = i + 1; j < nr_class; j++) {
                    final svm_problem sub_prob = new svm_problem();
    ...
    public static void svm_cross_validation(svm_problem prob,
        ...
        if ((param.svm_type == svm_parameter.C_SVC || param.svm_type == svm_parameter.NU_SVC) && nr_fold < l) {
            ...
            final int[] fold_count = new int[nr_fold];
            int c;
            ...
            for (c = 0; c < nr_class; c++) {
                for (i = 0; i < nr_fold; i++) {
                    final int begin = start[c] + i * count[c] / nr_fold;
                    final int end = start[c] + (i + 1) * count[c] / nr_fold;
        ...
        for (i = 0; i < nr_fold; i++) {
            ...
            final svm_model submodel = svm_train(subprob, param);
            if (param.probability == 1 && (param.svm_type == svm_parameter.C_SVC || param.svm_type == svm_parameter.NU_SVC)) {
                final double[] prob_estimates = new double[svm_get_nr_class(submodel)];
                for (j = begin; j < end; j++) {
        ...
        if (model.param.svm_type == svm_parameter.ONE_CLASS || model.param.svm_type == svm_parameter.EPSILON_SVR || model.param.svm_type == svm_parameter.NU_SVR) {
            ...
            for (i = 0; i < nr_class; i++) {
                for (int j = i + 1; j < nr_class; j++) {
                    ...
                    final int si = start[i];
                    final int sj = start[j];
                    final int ci = model.nSV[i];
                    final int cj = model.nSV[j];

                    int k;
                    final double[] coef1 = model.sv_coef[j - 1];
                    final double[] coef2 = model.sv_coef[i];
                    for (k = 0; k < ci; k++) {
    ...
    public static double svm_predict(svm_model model, svm_node[] x) {
        ...
        final double pred_result = svm_predict_values(model, x, dec_values);
        return pred_result;
    }
    ...
    public static void svm_save_model(String model_file_name, svm_model model) throws IOException {
        final DataOutputStream fp = new DataOutputStream(new BufferedOutputStream(new FileOutputStream(model_file_name)));

        final svm_parameter param = model.param;
        ...
        fp.writeBytes("SV\n");
        final double[][] sv_coef = model.sv_coef;
        final svm_node[][] SV = model.SV;
        ...
        for (int i = 0; i < l; i++) {
            ...
            final svm_node[] p = SV[i];
            if (param.kernel_type == svm_parameter.PRECOMPUTED) {
                fp.writeBytes("0:" + (int) (p[0].value));
            } else {
                for (final svm_node element : p) {
                    fp.writeBytes(element.index + ":" + element.value + " ");
                }
            }
            fp.writeBytes("\n");
        }
    ...
    public static svm_model svm_load_model(BufferedReader fp) throws IOException {
        ...
        while (true) {
            ...
            if (cmd.startsWith("svm_type")) {
                ...
                final int n = model.nr_class * (model.nr_class - 1) / 2;
