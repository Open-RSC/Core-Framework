public final class S_SymbolStringer extends Script {
        ...
        if (start_time == -1) {
            ...
            for (int i = 0; i < names.length; ++i) {
                if (getInventoryCount(unstrung_ids[i]) > 0) {
                    System.out.println("Stringing " + names[i] + " symbols");
    ...
    public void paint() {
        ...
        int y = 25;
        drawString("Symbol Stringer", 25, y, 4, white);
        y += 15;
        drawString("Running for " + _getRuntime(), 25, y, 1, white);
        for (int i = 0; i < names.length; ++i) {
            if (banked_counts[i] <= 0) continue;
            y += 15;
            drawString("Banked " + names[i] + " symbols: " + banked_counts[i],
                25, y, 1, white);
        }
        y += 15;
    }
    ...
    private void _printOut() {
        ...
        for (int i = 0; i < names.length; ++i) {
            if (banked_counts[i] <= 0) continue;
            System.out.println("Banked " + names[i] + " symbols: " + banked_counts[i]);
        }
    }
