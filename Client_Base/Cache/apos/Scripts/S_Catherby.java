...
public final class S_Catherby extends Script {
	...
	private static final int[] non_fish_net_loot = {
		779, 792, /* oysters */
	...
	public S_Catherby(Extension ex) {
		super(ex);
	...
	public static void main(String[] argv) {
		S_Catherby s = new S_Catherby(null);
		s.init(null);
	...
	public void onServerMessage(String str) {
		...
		if (str.contains("busy")) {
			...
			// special case for big netting since server may respond with nothing
	...
	private void _fishCsvOut() {
		try{
			fishcsv.write(
				...
				cur_attempts + "," +
				fish_success[0] + "," + fish_success[1] + "," + fish_success[2] + "\n"
			);
			fishcsv.flush();
		}
		catch(IOException e)
		...
		try{
			cookcsv.write(
				...
				if(fish_spots_ids[fish_cursor].length > 2) {
					cookcsv.write(
							fish_spots_ids[fish_cursor][2] + "," + 
							fish_spots_names[fish_cursor][2] + "," +
							getLevel(LEVEL_COOKING) + "," +
							(cook_success[2]+cook_failure[2]) + "," +
							cook_success[2] + "," + cook_failure[2] + "\n"
						);
				}
			cookcsv.flush();
	...
	public void paint() {
		...
		if (have_stage(STAGE_FISH)) {
			...
			y += 15;
			drawString(String.format(
			    "Successful fishing attempts: %s (%s/h), %s (%s/h), %s (%s/h), %s (%s/h)",
			    iformat.format(cur_fish_success),
			    per_hour(cur_fish_success, fish_level_up_time),
			    ...
			    iformat.format(fish_success[2]),
			    per_hour(fish_success[2], fish_level_up_time)),
			    x + 10, y, 2, white);
			y += 15;
			drawString(String.format(
		...
		if (have_stage(STAGE_COOK)) {
			...
			y += 15;
			drawString(String.format(
			    "Successful cooking attempts: %s (%s/h), %s (%s/h), %s (%s/h), %s (%s/h)",
			    iformat.format(cur_cook_success),
			    per_hour(cur_cook_success, cook_level_up_time),
			    ...
			    iformat.format(cook_success[2]),
			    per_hour(cook_success[2], cook_level_up_time)),
			    x + 10, y, 2, white);
			y += 15;
			drawString(String.format(
			    "Failed cooking attempts: %s (%s/h), %s (%s/h), %s (%s/h), %s (%s/h)",
			    iformat.format(cur_cook_fails),
			    ...
			    iformat.format(cook_failure[2]),
			    per_hour(cook_failure[2], cook_level_up_time)),
			    x + 10, y, 2, white);
			y += 15;
	...
	private void create_frame() {
		...
		final Checkbox drop_raw_sardine = new Checkbox(
			    "Special: Exclude sardine / drop after fishing",
			    false);
		...
		list.add("Anchovies/shrimp");
		list.add("Herring/sardine");
		list.add("Big net");
		list.add("Lobsters");
		list.add("Swordfish/tuna");
		list.add("Sharks");
		list.select(0);
		...
		ok.addActionListener(new ActionListener() {
			private void set_ids() {
				...
				case "Anchovies/shrimp":
					...
					fish_y = 500;
					click1 = true;
					if (drop_raw_shrimp.getState()) {
						raw_ids = new int[] { 351 };
						cooked_ids = new int[] { 352 };
				...
				case "Herring/sardine":
					...
					fish_y = 500;
					click1 = false;
					if (drop_raw_sardine.getState()) {
						raw_ids = new int[] { 361 };
						cooked_ids = new int[] { 362 };
						burnt_ids = new int[] { 360, 365 };
						discard_ids = new int[] { 354 };
					} else {
						raw_ids = new int[] {
						    354, 361
						};
				...
				case "Lobsters":
					...
					click1 = false;
					raw_ids = new int[] { 372 };
					cooked_ids = new int[] { 373 };
					burnt_ids = new int[] { 374 };
					discard_ids = new int[] {};
					break;
				case "Swordfish/tuna":
					fish_cursor = 1;
					...
					fish_y = 504;
					click1 = true;
					if (drop_raw_tuna.getState()) {
						raw_ids = new int[] { 369 };
						cooked_ids = new int[] { 370 };
						burnt_ids = new int[] { 371 };
				...
				case "Sharks":
					...
					click1 = false;
					raw_ids = new int[] { 545 };
					cooked_ids = new int[] { 546 };
					burnt_ids = new int[] { 547 };
					discard_ids = new int[] {};
					break;
				default:
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (big_netting) {
					if (cert.getState()) {
						System.out.println("NOTE: Big netting is a special case. When certing, everything (that doesn't get sold) other than bass will be banked.");
					} else if (sell_raw.getState()) {
						System.out.println("NOTE: Big netting is a special case. When selling raw fish, casket loot and pearls (if any) will be banked.");
					}
				}
